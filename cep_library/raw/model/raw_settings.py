...
class RawStatisticsBook:
    ...
    def update_with_stats(self, 
        ...
        self.local_write_time_ns_total.add_multiple(
            local_write_time_ns_total.mean,
            local_write_time_ns_total.count
        )
        self.local_write_size_byte_total.add_multiple(
            local_write_size_byte_total.mean,
            local_write_size_byte_total.count
        )
        self.local_write_data_count.add_multiple(
            local_write_data_count.mean,
            local_write_data_count.count
        )
        self.remote_write_time_ns_total.add_multiple(
            remote_write_time_ns_total.mean,
            remote_write_time_ns_total.count
        )
        self.remote_write_time_byte_total.add_multiple(
            remote_write_time_byte_total.mean,
            remote_write_time_byte_total.count
        )
        self.remote_write_data_count.add_multiple(
            remote_write_data_count.mean,
            remote_write_data_count.count
        )
    ...
    def get_calculated(self) -> float:
        return (self.local_write_time_ns_total.count + self.remote_write_time_ns_total.count) > 0
    ...
    def get_avg_byte_mul_ns(self, remote:bool) -> float:
        if remote:
            if self.remote_write_time_ns_total.count == 0:
                return ((self.local_write_time_ns_total.mean / 1000.0) * self.local_write_size_byte_total.mean) * configs.STATISTICS_REMOTE_MULTIPLIER
            return (self.remote_write_time_ns_total.mean / 1000.0) * self.remote_write_time_byte_total.mean
        else:
            if self.local_write_time_ns_total.count == 0:
                return ((self.remote_write_time_ns_total.mean / 1000.0) * self.remote_write_time_byte_total.mean) / configs.STATISTICS_REMOTE_MULTIPLIER
            return (self.local_write_time_ns_total.mean / 1000.0) * self.local_write_size_byte_total.mean

    def normalized_get_avg_byte_mul_ns_square(self, remote:bool):
        return self.normalize(self.minx, self.maxx, self.get_avg_byte_mul_ns_square(remote))

    def get_total_event_count(self) -> float:
        return self.local_write_data_count.total() + self.remote_write_data_count.total()
    ...
    def get_per_second_expected_event_count(self) -> float:
        ...
        if secs < 1.0:
            secs = 1.0
        return ((self.local_write_data_count.total() + self.remote_write_data_count.total()) * 1.0) / secs

    def get_avg_byte_mul_ns_square(self, remote:bool) -> float:
        if remote:
            if self.remote_write_time_ns_total.count == 0:
                return ((self.local_write_time_ns_total.mean / 1000.0)**2 * self.local_write_size_byte_total.mean) * configs.STATISTICS_REMOTE_MULTIPLIER
            return (self.remote_write_time_ns_total.mean / 1000.0)**2 * self.remote_write_time_byte_total.mean
        else:
            if self.local_write_time_ns_total.count == 0:
                return ((self.remote_write_time_ns_total.mean / 1000.0)**2 * self.remote_write_time_byte_total.mean) / configs.STATISTICS_REMOTE_MULTIPLIER
            return (self.local_write_time_ns_total.mean / 1000.0)**2 * self.local_write_size_byte_total.mean
    ...
    def get_avg_byte_over_n(self, remote:bool) -> float:
        if remote:
            if self.remote_write_time_ns_total.count == 0:
                return self.local_write_size_byte_total.mean * configs.STATISTICS_REMOTE_MULTIPLIER
            return self.remote_write_time_byte_total.mean
        else:
            if self.local_write_time_ns_total.count == 0:
                return self.remote_write_time_byte_total.mean / configs.STATISTICS_REMOTE_MULTIPLIER
    ...
    def get_avg_byte_over_ns(self, remote:bool) -> float:
        if remote:
            if self.remote_write_time_ns_total.count == 0:
                return (self.local_write_size_byte_total.mean / self.local_write_time_ns_total.mean) * configs.STATISTICS_REMOTE_MULTIPLIER
            
            return self.remote_write_time_byte_total.mean / self.remote_write_time_ns_total.mean
        else:
            if self.local_write_time_ns_total.count == 0:
                return (self.remote_write_time_byte_total.mean / self.remote_write_time_ns_total.mean) / configs.STATISTICS_REMOTE_MULTIPLIER
            return self.local_write_size_byte_total.mean / self.local_write_time_ns_total.mean
    ...
    def get_byte_per_data(self, remote:bool) -> float:
        if configs.DATA_LOAD_TYPE == 2:
            size = 1024.0 * (self.local_write_size_byte_total.total() + self.remote_write_time_byte_total.total()) / (self.local_write_data_count.count + self.remote_write_data_count.count)
        elif configs.DATA_LOAD_TYPE == 3:
            size = 1024.0 * (self.local_write_size_byte_total.total() + self.remote_write_time_byte_total.total()) / configs.evaluation_period
    ...
    def get_avg_ns_over_byte(self, remote:bool) -> float:
        if remote:
            if self.remote_write_time_ns_total.count == 0:
                return (self.local_write_time_ns_total.mean / self.local_write_size_byte_total.mean) * configs.STATISTICS_REMOTE_MULTIPLIER
            return self.remote_write_time_ns_total.mean / self.remote_write_time_byte_total.mean
        else:
            if self.local_write_time_ns_total.count == 0:
                return (self.remote_write_time_ns_total.mean / self.remote_write_time_byte_total.mean) / configs.STATISTICS_REMOTE_MULTIPLIER
            return self.local_write_time_ns_total.mean / self.local_write_size_byte_total.mean
    ...
    def get_avg_ns_over_n(self, remote:bool) -> float:
        if remote:
            if self.remote_write_time_ns_total.count == 0:
                return self.local_write_time_ns_total.mean * configs.STATISTICS_REMOTE_MULTIPLIER
            return self.remote_write_time_ns_total.mean
        else:
            if self.local_write_time_ns_total.count == 0:
                return self.remote_write_time_ns_total.mean / configs.STATISTICS_REMOTE_MULTIPLIER
    ...
    def get_write_size_byte_total(self) -> float:
        return self.remote_write_time_byte_total.mean + self.local_write_size_byte_total.mean
    ...
    def get_write_time_ns_total(self) -> float:
        return self.remote_write_time_ns_total.mean + self.local_write_time_ns_total.mean
        
    def add_raw_statistic(self, 
