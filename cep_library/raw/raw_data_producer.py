...
class RawDataProducer:
    def __init__(self, settings:RawSettings, db:DatabaseManagementService, app: FastAPI) -> None:
        
        self.database_management_one = db
        self.database_management_one.create_collection(settings.producer_name, 
    ...
    def register_action_file_service(self, app: FastAPI):
        @app.get("/raw_statistics_report/"+self.settings.output_topic.output_topic+"/")
        def raw_statistics_report():    
            ...
            written_count = usage.local_write_data_count.count + usage.remote_write_data_count.count
            print(f"[PRODUCER] {self.settings.raw_data_name} wrote count: {written_count}, total produced count: {self.produced_id_ctr}")
    ...
    def send(self, data, raw_data_tracker:str, data_priority:int=0):
        ...
        target_db_list:List[str] = deepcopy(self.settings.output_topic.target_databases)
        for target_db in target_db_list:
            ...
            if configs.MQTT_USE_PREFIX_FOR_EVENTS:
                for th in target_hosts:
                    fixed_output_topic = f"{self.settings.output_topic.output_topic}_{th}"
            ...
            event_received_duration:float = (datetime.now(timezone.utc) - raw_data_date).microseconds / 1000.0
            ...
            if target_db == self.settings.producer_name:
                local_write_time += data_helper.get_elapsed_ms(start_time) + event_received_duration
                local_data_size += payload_size
                local_data_count += 1
            else:
                remote_write_time += data_helper.get_elapsed_ms(start_time) + event_received_duration
    ...
    def update_data_target(self, msg: bytes, args): 
        ...
        for new_target_db in [n_t_db for n_t_db in target_update_request.output_topic.target_databases if n_t_db not in self.settings.output_topic.target_databases]:
            ...
            self.database_management_one.create_collection(new_target_db, 
                                                            target_update_request.output_topic.output_topic,
                                                            configs.collection_lifetime)
            
            
        
        self.settings.output_topic.target_databases = target_update_request.output_topic.target_databases
        self.settings.output_topic.target_hosts = target_update_request.output_topic.target_hosts
        ...
        if configs.DEBUG_MODE:
            print(f"[P] Target database update completed for {target_update_request.output_topic.output_topic}")
