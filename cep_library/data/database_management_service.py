...
class DatabaseManagementService:
    def __init__(self, host_name:str, groupid:str) -> None:
        ...
        db_client = self.get_mongo_client(configs.env_mongo_self, configs.env_mongo_port)
    ...
    def create_collection(self, host_name:str, collection_name:str, ttl_duration:int):
    ...
    def insert_one(self, data:dict, collection: str, preferred: str):
        try:
            ...
            if exc.timeout:
                ...
                if 'Cannot use MongoClient after close' in exc._message or 'BulkWriteError' in exc._message:
                    pass
                else:
            ...
            if exc.timeout:
                ...
                if 'Cannot use MongoClient after close' in exc._message or 'BulkWriteError' in exc._message:
                    ...
                    if exc.__class__.__name__ == 'BulkWriteError':
                        print(f"[CLIENT] [ERROR] [MONGO] Bulk write error has occured...")
                    else:
                        print(f"failed with non-timeout error: {exc!r}")
            ...
            with pymongo.timeout(configs.data_operation_timeout):
                ...
                if query.aggregate:
                    if len(query.aggregate) == 0:
                        raise Exception("Aggregate should have at least one aggregation in it!")
    ...
    def carry_data(self, data_carry_model:DataCarryModel):    
        ...
        source_data = self.query(data_carry_model.query, 
                                 data_carry_model.collection, 
                                 data_carry_model.source_host)
        
        
        if source_data:
            print(f"[DATABASE_MANAGEMENT_SERVICE] Data exists for migration, collection: {data_carry_model.collection}, source: {data_carry_model.source_host}, destination: {data_carry_model.target_host}, time: {datetime.now(tz=timezone.utc)}")
        ...
        if fin_migrated_count > 0:
            print(f"Migration completed with migrated count: {fin_migrated_count}, deleted count: {fin_deleted_count}")
