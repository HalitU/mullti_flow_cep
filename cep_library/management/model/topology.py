...
class TopologyNode:
    def __init__(
        ...
        node_id: int,
        node_type: NodeType,
        node_data: RawSettings | CEPTask | CEPConsumerSettings,
        name: str,
    ) -> None:
...
class Topology:
    ...
    def update_expected_flow_counts(
        self,
        ...
        for pn in self.producer_nodes:
            ...
            raw_expected_event_count = math.ceil(
                rawstats.get_expected_event_count(
                    n_d.producer_name, n_d.output_topic.output_topic
                )
            )
            print(
                f"Current raw expected count: {n_d.output_topic.output_topic}, {raw_expected_event_count}"
            )
            ...
            while len(successors) > 0:
                ...
                if (
                    succ.node_type == NodeType.RAWOUTPUT
                    or succ.node_type == NodeType.SINK
                ):
                    pass
                    ...
                    for rti in e_d.settings.required_sub_tasks:
                        for otl in output_topic_list:
                            ...
                            if rti.input_topic == otl:
                                ...
                                mssa.increment_expected_event_input_counts(
                                    e_d.settings.action_name,
                                    rti.input_topic,
                                    raw_expected_event_count,
                                )

                    mssa.increment_expected_event_output_counts(
                        e_d.settings.action_name,
                        e_d.settings.output_topic.output_topic,
                        raw_expected_event_count,
                    )
                    ...
                    for otl in output_topic_list:
                        if s_d.topic_name == otl:
                            if configs.LOGS_ACTIVE:
                                print(
                                    f"Processing consumer input topic: {s_d.topic_name} with expected: {raw_expected_event_count}"
                                )
                            consumerstats.increment_expected_event_counts(
                                s_d.host_name, s_d.topic_name, raw_expected_event_count
                            )
                            if configs.LOGS_ACTIVE:
                                print(
                                    f"consumer input topic: {s_d.topic_name} current expected: {consumerstats.get_expected_event_counts(s_d.host_name, s_d.topic_name)}"
                                )
    ...
    def get_producer_node(self, prod_name: str, raw_name: str) -> TopologyNode:
        return [
            p
            for p in self.producer_nodes
            if p.node_data.producer_name == prod_name
            and p.node_data.output_topic.output_topic == raw_name
        ][0]
    ...
    def add_edge(
        self, from_node: TopologyNode, to_node: TopologyNode, topic: str
    ) -> bool:
