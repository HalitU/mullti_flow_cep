...
class StatisticRecord:
    ...
    def update_miss_stats(self, ssr: SingleStatisticsRecord):
        ...
        for topic_name, vals in ssr.in_topic_records.items():
            if topic_name in self.in_topic_stats:
                self.in_topic_stats[topic_name].in_topic_local_read_byte.add(
                    vals.local_byte
                )
                self.in_topic_stats[topic_name].in_topic_local_read_ns.add(
                    vals.local_ns
                )
                self.in_topic_stats[topic_name].in_topic_local_data_count.add(
                    vals.local_data_count
                )

                self.in_topic_stats[topic_name].in_topic_remote_read_byte.add(
                    vals.remote_byte
                )
                self.in_topic_stats[topic_name].in_topic_remote_read_ns.add(
                    vals.remote_ns
                )
                self.in_topic_stats[topic_name].in_topic_remote_data_count.add(
                    vals.remote_data_count
                )
    ...
    def update_stats(self, ssr: SingleStatisticsRecord):
        ...
        self.crr_raw_elapsed_max_time_from_init_ms.add(
            ssr.raw_elapsed_max_time_from_init_ms
        )
        self.crr_raw_elapsed_min_time_from_init_ms.add(
            ssr.raw_elapsed_min_time_from_init_ms
        )
