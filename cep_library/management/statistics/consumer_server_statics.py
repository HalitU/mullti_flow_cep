...
class ConsumerServerStatics:
    ...
    def increment_expected_event_counts(self,  host_name:str, topic_name:str, count: float) -> None:
    ...
    def update_consumer_stats(self, consumer_stats:List[ConsumerTopicStats]) -> None:
        for cs in consumer_stats:
            if cs.calculated():
                if configs.UPDATE_LATEST_STATS and cs.host_name in self.consumer_stats and cs.topic_name in self.consumer_stats[cs.host_name]:
    ...
    def set_stat_min_max(self, host_name:str, topic_name:str, minx:float, maxx:float) -> None:
        if host_name in self.consumer_stats and topic_name in self.consumer_stats[host_name]:
            self.consumer_stats[host_name][topic_name].minx = minx
            self.consumer_stats[host_name][topic_name].maxx = maxx

    def normalized_get_reading_cp_sat_cost(self, host_name:str, topic_name:str, remote:bool) -> float:
        if host_name in self.consumer_stats and topic_name in self.consumer_stats[host_name]:
            return self.consumer_stats[host_name][topic_name].normalized_get_reading_cp_sat_cost(remote)
    ...
    def get_reading_cp_sat_cost(self, host_name:str, topic_name:str, remote:bool) -> float:
        if host_name in self.consumer_stats and topic_name in self.consumer_stats[host_name]:
            return self.consumer_stats[host_name][topic_name].get_reading_cp_sat_cost(remote)
    ...
    def normalize_get_reading_ns_over_byte(self, host_name:str, topic_name:str, remote:bool) -> float:
        if host_name in self.consumer_stats and topic_name in self.consumer_stats[host_name]:
            return self.consumer_stats[host_name][topic_name].normalize_get_reading_ns_over_byte(remote)
    ...
    def get_expected_load_per_second(self, host_name:str, topic_name:str, remote:bool) -> float:
        if configs.EXPECTED_LOAD_TYPE == 1: 
            expected_count_per_sec = self.get_expected_event_counts(host_name, topic_name)
    ...
    def get_reading_ns_over_byte(self, host_name:str, topic_name:str, remote:bool) -> float:
        if host_name in self.consumer_stats and topic_name in self.consumer_stats[host_name]:
            return self.consumer_stats[host_name][topic_name].get_reading_ns_over_byte(remote)
    ...
    def normalize_get_reading_byte_mul_ns(self, host_name:str, topic_name:str, remote:bool) -> float:
        if host_name in self.consumer_stats and topic_name in self.consumer_stats[host_name]:
            return self.consumer_stats[host_name][topic_name].normalize_get_reading_byte_mul_ns(remote)
    ...
    def get_reading_byte_mul_ns(self, host_name:str, topic_name:str, remote:bool) -> float:
        if host_name in self.consumer_stats and topic_name in self.consumer_stats[host_name]:
            return self.consumer_stats[host_name][topic_name].get_reading_byte_mul_ns(remote)
    ...
    def normalize_get_reading_byte_mul_ns_square(self, host_name:str, topic_name:str, remote:bool) -> float:
        if host_name in self.consumer_stats and topic_name in self.consumer_stats[host_name]:
            return self.consumer_stats[host_name][topic_name].normalize_get_reading_byte_mul_ns_square(remote)
        
        default_val:TopicStatBase = TopicStatBase()
        for _, ts in self.consumer_stats.items():
            for _, stat in ts.items():
                default_val.add_value(stat.normalize_get_reading_byte_mul_ns_square(remote))
        return default_val.get_default_val()
    
    def get_reading_byte_mul_ns_square(self, host_name:str, topic_name:str, remote:bool) -> float:
        if host_name in self.consumer_stats and topic_name in self.consumer_stats[host_name]:
            return self.consumer_stats[host_name][topic_name].get_reading_byte_mul_ns_square(remote)
