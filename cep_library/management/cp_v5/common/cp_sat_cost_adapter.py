    ...
    def adaptCosts(self, 
        ...
        for task in cep_tasks:
            ...
            for rst in task.settings.required_sub_tasks:
                load_cost: float = mssa.get_expected_topic_load_per_second(task.settings.action_name, rst.input_topic, False)
                min_cost = min(min_cost, load_cost)
                max_cost = max(max_cost, load_cost)
        ...
        for _, raw_host_settings in producer_info.items():
            for _, raw_s in raw_host_settings.items():
                producer_stats.set_stat_min_max(raw_s.producer_name, raw_s.output_topic.output_topic, min_cost, max_cost)

        for task in cep_tasks:
            mssa.set_event_stat_min_max(task.settings.action_name, min_cost, max_cost)
            
            for rts in task.settings.required_sub_tasks:
                mssa.set_topic_stat_min_max(task.settings.action_name, rts.input_topic, min_cost, max_cost)

        for consumer_host, consumer_topics in consumer_info.items():
            for topic_name, _ in consumer_topics.items():
                consumer_stats.set_stat_min_max(consumer_host, topic_name, min_cost, max_cost)

        print("-----------------------------------------------------------")
        print(f"[SERVER] Minimum and maximum costs were: {min_cost}-{max_cost}, normalized between: {self.desired_cost_lower}-{self.desired_cost_upper}")
        print(f"[SERVER] Min is normalized to: {self.normalize(min_cost, max_cost, min_cost)}")
        print(f"[SERVER] Max is normalized to: {self.normalize(min_cost, max_cost, max_cost)}")
        print("-----------------------------------------------------------")
