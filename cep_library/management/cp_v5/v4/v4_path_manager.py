...
class CPSATV4PathManager:
    ...
    def objective_func(self, 
        ...
        if configs.CP_USE_MAX_CONNECTION_LOAD_COST:
            ...
            for _, vals in self.path_sums.items():
                ...
                duration:cp_model.IntVar = self.model.NewIntVar(self.minc, self.maxc, "bottleneck")
                ...
                if configs.CP_USE_DAG_BOTTLENECK:
                    bottleneck:cp_model.IntVar = self.model.NewIntVar(self.minc, self.maxc, "bottleneck")
        ...
        for raw_id, raw_topics in self.variables.raw_source_target_costs.items():
            ...
            for raw_topic, raw_topic_devices in raw_topics.items():
                ...
                if not configs.KEEP_RAWDATA_AT_SOURCE:
                    crr_raw_topic_link_cost:cp_model.IntVar = self.model.NewIntVar(self.minc, self.maxc, f"raw:{raw_id}:{raw_topic}")
                ...
                if raw_topic not in self.related_events:
                    self.related_events[raw_topic] = [event for _, event in self.events.items() if event.topic_exists(raw_topic)]
                related_events = self.related_events[raw_topic]

                
                for re in related_events:
                    self.traverse_reading_path_costs(costs, re.event_id, raw_topic, path_id)

                
                costs = []

    def traverse_reading_path_costs(self, accumulative_costs:List[cp_model.IntVar], event_id:str, topic_id:str, path_id:str):
        
        
        node_id:str = f"{event_id}:{topic_id}"
        if node_id not in self.traversal_records:
            costs:List[cp_model.IntVar] = self.variables.event_reading_location_costs[event_id][topic_id]
            crr_topic_reading_cost:cp_model.IntVar = self.model.NewIntVar(self.minc, self.maxc, "c")
    ...
    def traverse_writing_path_costs(self, accumulative_costs:List[cp_model.IntVar], event_id:str, path_id:str):
        
        
        for topic_id, out_t_devices in self.variables.event_output_location_costs[event_id].items():
            
            node_id = f"{event_id}:{topic_id}"
            if node_id not in self.traversal_records:
                crr_out_topic_cost:cp_model.IntVar = self.model.NewIntVar(self.minc, self.maxc, f"e_write:{event_id}:{topic_id}")
            ...
            if topic_id not in self.related_events:
                self.related_events[topic_id] = [event for _, event in self.events.items() if event.topic_exists(topic_id)]
            ...
            for re in related_events:        
                ...
                self.traverse_reading_path_costs(accumulative_costs, re.event_id, topic_id, path_ref)
            ...
            self.traverse_consumer_reading_path_cost(accumulative_costs, topic_id, path_ref)
            
            del accumulative_costs[-1]

    def traverse_consumer_reading_path_cost(self, accumulative_costs:List[cp_model.IntVar], topic:str, path_id:str):
        ...
        for consumer_id, consumer_topics in self.consumers.items():
            ...
            if node_id not in self.traversal_records:
                costs:List[cp_model.IntVar] = []
                for _, cs in self.variables.event_consumer_reading_costs[consumer_id][topic].items():
                    costs.append(cs)
                consumer_reading_cost:cp_model.IntVar = self.model.NewIntVar(self.minc, self.maxc, f"cons:{consumer_id}:{topic}")
            ...
            print(f"Consumer: {consumer_id}:{topic}, number of accumulative costs: {len(accumulative_costs)}")
