                ...
                for written_device_id, written_to_device in raw_output_targets.items():
                    ...
                    for rn in rel_nodes:
                        if rn.node_type == NodeType.EVENTEXECUTION:
                            self.variables.event_reading_locations[rn.name][
                                raw_topic_id
                            ][written_device_id] = written_to_device
                        elif rn.node_type == NodeType.CONSUMER:
                            self.variables.event_consumer_location_vars[rn.name][
                                raw_topic_id
                            ][written_device_id] = written_to_device
                ...
                for d_id, d_var in t_devices.items():
                    ...
                    for rn in rel_nodes:
                        if rn.node_type == NodeType.EVENTEXECUTION:
                            ...
                            self.variables.event_reading_locations[rn.name][topic_id][
                                d_id
                            ] = d_var
                        elif rn.node_type == NodeType.CONSUMER:

                            cn_data: CEPConsumerSettings = rn.node_data
                            self.variables.event_consumer_location_vars[
                                cn_data.host_name
                            ][topic_id][d_id] = d_var
                        else:
