...
class CPSATV5FlowOptimizer:
    ...
    def optimize_flow(self, 
        ...
        cca.adaptCosts(producers,rawstats, cep_tasks, mssa, event_consumers, consumerstats)
        ...
        for raw_host, raw_settings in producers.items():
            for _, prd in raw_settings.items():
                ...
                p_top_node:TopologyNode = topology.get_producer_node(raw_host, prd.output_topic.output_topic)
                ...
                if "raw_sensors" in old_distribution_history and new_source.get_key() in old_distribution_history["raw_sensors"]:
                    for _, raw_output_topic_devices in old_distribution_history["raw_sensors"][new_source.get_key()].items():
                ...
                if len(historical_currently_stored_devices) > 0:
                    new_source.currently_stored_devices = historical_currently_stored_devices
                    
                else:
                    print(f"No currently stored device for raw: {new_source.device_id} - {new_source.settings.output_topic.output_topic}")
        ...
        for exec_n in topology.get_topologically_ordered_executor_nodes():
            ...
            if "event_sensors" in old_distribution_history:
                if len(list(old_distribution_history["event_sensors"].keys())) > 0:
                    for hist_worker_id, hist_events in old_distribution_history["event_sensors"].items():
                        if exec_n.name in hist_events:
                            event_historical_data[hist_worker_id] = hist_events[exec_n.name]
        ...
        total_link_cost, max_link_cost, worker_device_uplink_vars, worker_device_downlink_vars, max_connection_cost = cpv.prepareBandwidthLimitBalancer()
        ...
        normalized_bw_limit:float = configs.CUMULATIVE_LOAD_LIMIT / configs.CP_SAT_DIVIDER
        ...
        if configs.CP_USE_TWO_STEP_DAG_LINK_LOAD_COST:
            ...
            print("===========================")
            print(f"Max recorded connection load in the previous step was: {max_conn_cost_val}")
            print(f"Max recorded connection load ratio was: {(max_conn_cost_val * 100.0) / (normalized_bw_limit / 2.0)}")
            print("===========================")
        ...
        for d_id, d_loads in worker_device_uplink_vars.items():
            ...
            CPSATSTATS.max_recorded_uplink_load = max(CPSATSTATS.max_recorded_uplink_load, total)
            
            print(f"Device: {d_id}, count: {len(d_loads)} external link cost total: {total}, iis over limit: {over_limit}, link count: {n_links}")
        ...
        for d_id, d_loads in worker_device_downlink_vars.items():
            ...
            CPSATSTATS.max_recorded_downlink_load = max(CPSATSTATS.max_recorded_downlink_load, total)
        ...
        for psm, psm_values in cpm.path_sums.items():
            ...
            CPSATSTATS.max_recorded_path_duration = max(CPSATSTATS.max_recorded_path_duration, total)
        ...
        print(f"Maximum recoded downlink load was: {CPSATSTATS.max_recorded_downlink_load}")
        print(f"Maximum recoded path duration was: {CPSATSTATS.max_recorded_path_duration}")
        print("================================================")
        ...
        event_alterations, raw_updates, distribution_history, __, consumer_alterations = dist.determine_distributions()
        ...
        return event_alterations, raw_updates, consumer_alterations, distribution_history, max_load_perc
