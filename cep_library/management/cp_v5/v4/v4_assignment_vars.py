...
class CPSATV4AssignmentVariables:
    def __init__(
        ...
        self.raw_source_target_costs: dict[
            str, dict[str, dict[str, cp_model.IntVar]]
        ] = {}

        self.event_reading_locations: dict[
            str, dict[str, dict[str, cp_model.IntVar]]
        ] = {}
        self.event_reading_location_costs: dict[
            str, dict[str, List[cp_model.IntVar]]
        ] = {}

        self.event_execution_location_vars: dict[str, dict[str, cp_model.IntVar]] = {}

        self.event_output_location_vars: dict[
            str, dict[str, dict[str, cp_model.IntVar]]
        ] = {}
        self.event_output_location_costs: dict[
            str, dict[str, List[cp_model.IntVar]]
        ] = {}

        self.event_consumer_location_vars: dict[
            str, dict[str, dict[str, cp_model.IntVar]]
        ] = {}
        self.event_consumer_reading_costs: dict[
            str, dict[str, dict[str, cp_model.IntVar]]
        ] = {}
    ...
    def raw_source_output_targets(
        self, worker_device_ids: list[str], raws: dict[str, CPSATV3RawSource]
    ):
        n_vars = 0
        for raw_source_id, raw_setting in raws.items():
            ...
            for raw_output_topic in topics:
                ...
                for worker_device_id in worker_device_ids:
                    ...
                    self.raw_source_targets[raw_source_id][raw_output_topic][
                        worker_device_id
                    ] = new_raw_source_target

        return n_vars

    def prepare_event_reading_locations(
        self, worker_device_ids: List[str], events: dict[str, CPSATV3EventInfo]
    ):
        n_vars = 0
    ...
    def event_execution_locations(
        ...
        for event_id in events:
            ...
            for worker_device_id in worker_device_ids:
                new_event_execution_location: cp_model.IntVar = self.model.NewBoolVar(
                    "v"
                )
                n_vars += 1
                current_event_execution_locations[worker_device_id] = (
                    new_event_execution_location
                )

            self.event_execution_location_vars[event_id] = (
                current_event_execution_locations
            )

        return n_vars

    def event_output_locations(
        self, worker_device_ids: List[str], events: dict[str, CPSATV3EventInfo]
    ):
        n_vars = 0
            ...
            for output_topic in topics:
                ...
                for d_id in worker_device_ids:
                    ...
                    if configs.CP_SAT_WRITE_AT_EXECUTION_LOC:
                        exec_at_loc: cp_model.IntVar = (
                            self.event_execution_location_vars[e_id][d_id]
                        )
                        self.model.Add(exec_at_loc == new_topic_output_location)

                    self.event_output_location_vars[e_id][output_topic][
                        d_id
                    ] = new_topic_output_location

        return n_vars

    def event_consumer_locations(
        self,
        worker_device_ids: List[str],
        consumers: dict[str, dict[str, CEPConsumerSettings]],
    ):
