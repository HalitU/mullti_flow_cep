...
class CPSATV4Distribuion:
    ...
    def determine_raw_distributions(self, 
                                    connection_validation_graph:nx.DiGraph, 
                                    source:int,
                                    distribution_history:dict[str, dict[str, dict[str, list[str]]]] ,
                                    producer_updates:List[RawUpdateEvent]) -> int:
        n_raw_manipulation_created = 0
        for raw_source_id, raw_output_topics in self.variables.raw_source_targets.items():
            ...
            for raw_output_topic, raw_topic_devices in raw_output_topics.items():
                raw_source_topic_target_devices:List[str] = [k for k, v in raw_topic_devices.items() if self.solver.Value(v) == 1]

                
                
                if not raw_source_topic_target_devices:
                    raise Exception("All raw data sources are required to have an output location.")

                
                
                if raw_source_id not in distribution_history["raw_sensors"]:
                    distribution_history["raw_sensors"][raw_source_id] = {}
                distribution_history["raw_sensors"][raw_source_id][raw_output_topic] = raw_source_topic_target_devices

                

                
                for rot in raw_source_topic_target_devices:
                    connection_validation_graph.add_edge(raw_source_id, (rot, raw_output_topic))

                
                
                
                raw_setting.settings.output_topic.target_databases = raw_source_topic_target_devices
                producer_target_db_update = RawUpdateEvent()
                producer_target_db_update.raw_name = raw_setting.settings.raw_data_name
                producer_target_db_update.producer_name = raw_setting.settings.producer_name
                producer_target_db_update.output_topic = raw_setting.settings.output_topic
    ...
    def determine_execution_distributions(self,
                                        connection_validation_graph:nx.DiGraph,
                                        alterations: List[TaskAlterationModel],
                                        distribution_history: dict[str, dict[str, dict[str, dict[str, dict[str, list[str]]]]]]
                                        ) -> tuple[int, int]:
        
        n_tasks_activated = 0
        n_tasks_deactivated = 0
        for event_id, event_executors in self.variables.event_execution_location_vars.items():
            event_executor_devices: List[str] = [e for e, v in event_executors.items() if self.solver.Value(v) == 1]
            ...
            for rt in event_task.settings.required_sub_tasks:
                ...
                reading_choices:List[str] = [e for e, v in self.variables.event_reading_locations[event_id][rt.input_topic].items() if self.solver.Value(v) == 1]
                print(f"Reading choices for event: {event_id}, topic: {rt.input_topic} are: {reading_choices}")
                ...
                distribution_history["event_sensors"][event_executor_device][event_id]["input_topics"][rt.input_topic] = device_choices
    
                
                for eetit in device_choices:
                    connection_validation_graph.add_edge((eetit, rt.input_topic), (event_executor_device, event_id))
            
            
            
            output_topic_name:str = event_task.settings.output_topic.output_topic
            activated_target_locations:List[str] = [k for k, v in self.variables.event_output_location_vars[event_id][output_topic_name].items() if self.solver.Value(v) == 1]
            
            if len(activated_target_locations) == 0:
                raise Exception("Number of activated devices cannot be 0!")
            
            
            event_task.settings.output_topic.target_databases = activated_target_locations

            
            distribution_history["event_sensors"][event_executor_device][event_id]["output_topics"][output_topic_name] = activated_target_locations

            

            
            for eetot in activated_target_locations:
                connection_validation_graph.add_edge((event_executor_device, event_id), (eetot, output_topic_name))
        ...
        print(f"Execution assignments are completed, activated: {n_tasks_activated}, deactivated: {n_tasks_deactivated}.")
        
        return n_tasks_activated, n_tasks_deactivated

    def validate_assignment_dag(self, connection_validation_graph:nx.DiGraph, source:int, sink:int):
        ...
        if len(nx.bfs_tree(connection_validation_graph, source)) != connection_validation_graph.number_of_nodes():
            raise Exception(f"Not all nodes are between the source and the sink, number of nodes: {connection_validation_graph.number_of_nodes()}, current number of connected nodes: {len(nx.bfs_tree(connection_validation_graph, source))}")
    ...
    def determine_raw_migration_assignments(self, alterations: List[TaskAlterationModel]):
        
        n_migration_requests = 0
        for _, raw_setting in self.raws.items():
            new_devices:List[str] = self.output_topic_records[raw_setting.settings.output_topic.output_topic]
            ...
            for csd in raw_setting.currently_stored_devices:
                ...
                if csd not in new_devices:
                    ...
                    chosen_device:str = np.random.choice(a=new_devices, size=1, replace=False)[0]
                    
                    
                    
                    
                    migration_request = DataMigrationModel()
                    migration_request.topic = raw_setting.settings.output_topic.output_topic
    ...
    def determine_event_data_migrations(self, alterations: List[TaskAlterationModel]):
        ...
        for _, event_info in self.events.items():
            currently_written_device:list[str] = event_info.get_output_topic_currently_written_devices()
            newly_written_devices: List[str] = self.output_topic_records[event_info.cep_task.settings.output_topic.output_topic]
            ...
            for cwd in currently_written_device:
                ...
                if cwd not in newly_written_devices:
                    ...
                    chosen_device:str = np.random.choice(a=newly_written_devices, size=1, replace=False)[0]
                    ...
                    migration_request = DataMigrationModel()
                    migration_request.topic = event_info.cep_task.settings.output_topic.output_topic
                    migration_request.current_host = cwd
                    migration_request.target_host = chosen_device
                    migration_request.data_type = 1
                    migration_request.event_name = event_info.cep_task.settings.action_name
    ...
    def determine_consumer_reading_locations(self, consumer_alterations: List[CEPConsumerUpdateEvent],
                                             connection_validation_graph:nx.DiGraph):
        n_consumer_requests = 0
        
        for consumer_id, consumer_topics in self.variables.event_consumer_location_vars.items():
            for topic_id, topic_reading_devices in consumer_topics.items():
                reading_devices: List[str] = [d for d, v in topic_reading_devices.items() if self.solver.Value(v) == 1]
                
                if len(reading_devices) == 0:
                    raise Exception("Each consumer has to read its topic data from a device!!!!!!")
                ...
                for eetit in reading_devices:
                    ...
                    connection_validation_graph.add_edge((eetit, topic_id), (consumer_id, f"{consumer_id}_{topic_id}"))
    ...
    def determine_distributions(self):
        ...
        distribution_history: dict[str, dict[str, dict[str, list[str]]]] |  dict[str, dict[str, dict[str, dict[str, dict[str, list[str]]]]]] = {
            "raw_sensors": {},
            "event_sensors": {}
        }
        ...
        n_tasks_activated, n_tasks_deactivated = self.determine_execution_distributions(
            connection_validation_graph, 
            alterations, 
            distribution_history)

        
        n_consumer_requests = self.determine_consumer_reading_locations(consumer_alterations, connection_validation_graph)
