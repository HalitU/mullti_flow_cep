...
def random_distribution(
    ...
    for _, raw_settings in producers.items():
        for _, producer in raw_settings.items():
            ...
            chosen_worker_device_ids: List[str] = np.random.choice(
                a=worker_device_ids,
                size=configs.ALLOWED_PARALLEL_EXECUTION_COUNT,
                replace=False,
            )
    ...
    for task_node in task_nodes:
        ...
        for execution_host in chosen_worker_device_ids:
            ...
            for c in copied_task.settings.output_topic.target_databases:
                if (
                    c
                    not in topic_targets[copied_task.settings.output_topic.output_topic]
                ):
                    topic_targets[
