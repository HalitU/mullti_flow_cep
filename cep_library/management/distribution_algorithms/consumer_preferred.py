...
def consumer_preferred(
    ...
    for _, raw_settings in producers.items():
        for _, producer in raw_settings.items():
            ...
            if len(producer.output_topic.target_databases) > 1:
                raise Exception(
                    "Consumer approach should be writing an output to a single location only!"
                )
            ...
            if target_topic_id not in topic_outputs[producer.output_topic.output_topic]:
                topic_outputs[producer.output_topic.output_topic].append(
                    target_topic_id
                )
    ...
    for task_node in ordered_nodes:
        ...
        device_choice: str = event_to_be_executed_devices[
            task_data.settings.action_name
        ]
        ...
        if (
            target_topic_id
            not in topic_outputs[copied_task.settings.output_topic.output_topic]
        ):
            topic_outputs[copied_task.settings.output_topic.output_topic].append(
                target_topic_id
            )
...
def get_topic_to_be_read_devices(
...
def prepare_topic_device_targets(
    ...
    for consumer_id, consumer_topics in event_consumers.items():
        ...
        for topic_id, _ in consumer_topics.items():
            consumer_total_load += consumerstats.get_expected_load_per_second(
                consumer_id, topic_id, False
            )
        for topic_id, _ in consumer_topics.items():
            ...
            if consumer_id not in list(topic_to_be_read_devices[topic_id].keys()):
                ...
                if (
                    consumer_total_load
                    > topic_to_be_read_devices[topic_id][consumer_id]
                ):
                    topic_to_be_read_devices[topic_id][
                        consumer_id
                    ] = consumer_total_load
    ...
    for task_node in ordered_nodes:
        ...
        for rti in task_data.settings.required_sub_tasks:
            task_total_load += mssa.get_expected_topic_load_per_second(
                task_data.settings.action_name, rti.input_topic, False
            )

        device_choices: dict[str, float] = topic_to_be_read_devices[
            task_data.settings.output_topic.output_topic
        ]
        ...
        if len(available_devices) > 0:
            ...
            available_devices = dict(
                sorted(
                    available_devices.items(), key=lambda item: item[1], reverse=True
                )
            )
            ...
            random_device_choices: List[str] = np.random.choice(
                a=valid_devices, size=1, replace=False
            )
        ...
        for rti in task_data.settings.required_sub_tasks:
            ...
            worker_execution_loads[
                ...
                task_data.settings.action_name, rti.input_topic
            )
        ...
        for related_events in related_event_nodes:
            if related_events.node_type == NodeType.EVENTEXECUTION:
                ...
                if (
                    related_event_data.settings.action_name
                    in event_to_be_executed_devices
                ):
                    other_device_id: str = event_to_be_executed_devices[
                        related_event_data.settings.action_name
                    ]
            ...
            if related_events.node_type == NodeType.CONSUMER:
                ...
                link_load: float = consumerstats.get_expected_load_per_second(
                    related_consumer_data.host_name,
                    task_data.settings.output_topic.output_topic,
                    False,
                )
        ...
        for rt in task_data.settings.required_sub_tasks:
            ...
            if device_choice not in list(
                topic_to_be_read_devices[rt.input_topic].keys()
            ):
                topic_to_be_read_devices[rt.input_topic][
                    device_choice
                ] = task_total_load
                ...
                if (
                    task_total_load
                    > topic_to_be_read_devices[rt.input_topic][device_choice]
                ):
                    topic_to_be_read_devices[rt.input_topic][
                        device_choice
                    ] = task_total_load
