    ...
    for task_alteration in alterations:
        ...
        for rt in task_alteration.cep_task.settings.required_sub_tasks:
            ...
            for sub_alteration in alterations:
                ...
                if sub_alteration.cep_task.settings.output_topic.output_topic == rt.input_topic:
                    if fixed_device_choice not in sub_alteration.cep_task.settings.output_topic.target_hosts:
                        sub_alteration.cep_task.settings.output_topic.target_hosts.append(fixed_device_choice)
        ...
        for cons_alteration in consumer_alterations:
            if cons_alteration.topic_name == task_alteration.cep_task.settings.output_topic.output_topic:
                if cons_alteration.host not in task_alteration.cep_task.settings.output_topic.target_hosts:
                    task_alteration.cep_task.settings.output_topic.target_hosts.append(cons_alteration.host)
                    print(f"Appending consuemr target: {cons_alteration.host} for event: {task_alteration.job_name}")
...
def producer_preferred(
    ...
    ordered_nodes: List[TopologyNode] = topology.get_topologically_ordered_executor_nodes()
    ...
    for task_node in ordered_nodes:
        ...
        for rt in task_data.settings.required_sub_tasks:
            link_load: float = mssa.get_expected_topic_load_per_second(task_data.settings.action_name, rt.input_topic, False)
        ...
        if len(list(device_choices.keys())) == 0:
            ...
            random_device_choices = np.random.choice(a=get_devices_without_limit_final(
                task_data,
                mssa,
                topic_outputs,
                event_consumers,
            ...
            device_choices = dict(sorted(device_choices.items(), key=lambda item: item[1], reverse=True))
        ...
        for rt in task_data.settings.required_sub_tasks:
            ...
            worker_execution_loads[device_choice] += mssa.get_expected_event_input_counts(task_data.settings.action_name, rt.input_topic)

        
        for consumer_id, consumer_topics in event_consumers.items():
            for topic_id, consumer_setting in consumer_topics.items():
                link_load: float = consumerstats.get_expected_load_per_second(consumer_setting.host_name, topic_id, False)
                if topic_id == task_data.settings.output_topic.output_topic and consumer_setting.host_name != device_choice:
                    worker_external_loads[device_choice] += link_load
                    worker_external_loads[consumer_setting.host_name] += link_load
