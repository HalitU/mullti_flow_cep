...
from cep_library.management.statistics.load_helper import get_estimated_data_delay, get_max_percentage
...
class SpringEventElement:
    ...
    def input_topic_exists(self, topic:str) -> bool:
        return topic in [rs.input_topic for rs in self.cep_task.settings.required_sub_tasks]
...
class SpringNode:
    def __init__(self, node_type:SpringElemenType, d_id:str, ele:Any
                 ) -> None:
...
class SpringGraph:
    ...
    def get_size(self, print_log:bool=False) -> float:
        ...
        max_bw_load, above_threshold_count, total_load_sum, total_load_min, total_load_max, link_load_sum, link_load_max = self.get_highest_bw_load(print_loads=print_log)
    ...
    def device_limit_reached(self, d_id:str, skip_load_check:bool=False) -> bool:
        device_cpu_loads, device_uplink_load, device_downlink_load = self.get_entire_load()
        
        limit_reached:bool = (d_id in device_cpu_loads and device_cpu_loads[d_id] >= configs.DEVICE_EXECUTION_LIMIT) or \
            (configs.RELAX_USE_HARD_BW_LIMIT and \
                (
                    (d_id in device_uplink_load and device_uplink_load[d_id] > (configs.CUMULATIVE_LOAD_LIMIT/2)) \
                    or  (d_id in device_downlink_load and device_downlink_load[d_id] > (configs.CUMULATIVE_LOAD_LIMIT/2))
                ) \
    ...
    def add_executor_node(self, node:SpringNode):
        ...
        if self.event_counts[ele.event_id] >= configs.ALLOWED_PARALLEL_EXECUTION_COUNT:
            raise Exception("Should not assign an event beyond its parallelization limit!")
    ...
    def remove_executor_node(self, node:SpringNode):
        ...
        if self.device_limits[node.d_id] < 0 or self.event_counts[ele.event_id] < 0 or self.node_count < 0:
            raise Exception("Counts cannot be below 0!!!")
        
    

    def add_edge(self, source:SpringNode, target:SpringNode, weight:float, topic:str, expected_event_byte:float):
        if weight is None or weight == 0:
            raise Exception("Weight cannot be none!")
        if topic is None or topic == "":
            raise Exception("Weight cannot be none!")
        if source.d_id != target.d_id and expected_event_byte == 0:
            raise Exception("Devices are different but expected_event_byte is equal to 0!")
        
        self.G.add_edge(source, target, weight=weight, topic=topic, expected_event_byte=expected_event_byte)

    def the_bw_ratio(self, ratio:float) -> float:
        value:float = 1 + max(0, 1 + math.floor((ratio - 0.75) / 0.1)) * configs.GA_OVERLOAD_PENALTY
        ...
        for u, v, data_dict in self.G.edges.data(data=True):
            if "expected_event_byte" in data_dict:
                ...
                if u.d_id != v.d_id and load == 0.0:
                    raise Exception(f"Devices are different but load is equal to 0, {u.d_id}:{u.node_type}, {v.d_id}:{v.node_type}, {load}")
    ...
    def get_entire_load(self) -> Tuple[dict[str, float], dict[str, float], dict[str, float]]:
        ...
        for u in list(nx.topological_sort(self.G)):
            ...
            if u.node_type == SpringElemenType.RAWOUTPUT:
                ...
                device_cpu_loads[u.d_id] += self.rawstats.get_expected_event_count(raw_ele.settings.producer_name, raw_ele.settings.output_topic.output_topic)
            if u.node_type == SpringElemenType.CONSUMER:
                cons_ele: SpringConsumerElement = u.ele
                device_cpu_loads[u.d_id] += self.consumerstats.get_expected_event_counts(cons_ele.settings.host_name, cons_ele.settings.topic_name)
            if u.node_type == SpringElemenType.EVENTEXECUTION:
                ...
                for rti in exec_ele.cep_task.settings.required_sub_tasks:
                    device_cpu_loads[u.d_id] += self.mssa.get_expected_event_input_counts(exec_ele.cep_task.settings.action_name, rti.input_topic)
    ...
    def get_expected_delay(self, 
        ...
        return min_link_duration + (link_load - min_link_load) * (max_link_duration - min_link_duration) / (max_link_load - min_link_load)

    def get_highest_bw_load(self, print_loads:bool=False) -> Tuple[float, float, float, float, float, float, float]:
        device_cpu_loads:dict[str, float] = {}
        u:SpringNode
        for u in list(nx.topological_sort(self.G)):
            if u.d_id not in device_cpu_loads:
            ...
            if "expected_event_byte" in data_dict:
                ...
                if u.d_id != v.d_id:
                    ...
                    if configs.RELAX_USE_RATE_ONLY_FOR_MAX_LINK:
                        ...
                        u_load_rate: float = load / (configs.CUMULATIVE_LOAD_LIMIT / device_link_count[u.d_id])
                        v_load_rate: float = load / (configs.CUMULATIVE_LOAD_LIMIT / device_link_count[v.d_id])
                        ...
                        u_min_duration: float = device_min_loads[u.d_id] / (configs.CUMULATIVE_LOAD_LIMIT / device_link_count[u.d_id])
                        v_min_duration: float = device_min_loads[v.d_id] / (configs.CUMULATIVE_LOAD_LIMIT / device_link_count[v.d_id])
                        ...
                        if configs.RELAX_USE_ADVANCED_LINK_COST_WITH_LOAD_PENALTY == True:
                            u_load_rate = u_load_rate * device_over_limits[u.d_id]
                            v_load_rate = v_load_rate * device_over_limits[v.d_id]
                        ...
                        u_load_rate: float = load / (configs.CUMULATIVE_LOAD_LIMIT / device_uplink_count[u.d_id])
                        v_load_rate: float = load / (configs.CUMULATIVE_LOAD_LIMIT / device_downlink_count[v.d_id])
                        
                        max_delay:float = max(u_load_rate, v_load_rate)
                    elif configs.RELAX_USE_UP_DOWN_LINK_LOAD:
                        u_load_rate: float = load / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                        v_load_rate: float = load / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                        
                        u_device_load: float = device_uplink_load[u.d_id] / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                        v_device_load: float = device_downlink_load[v.d_id] / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                        ...
                        u_load_rate: float = device_uplink_load[u.d_id] / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                        v_load_rate: float = device_downlink_load[v.d_id] / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                        ...
                        if device_uplink_load[u.d_id] > (configs.CUMULATIVE_LOAD_LIMIT / 2.0):
                            u_load_rate = u_load_rate * 1000000000.0
                        elif device_uplink_load[u.d_id] > (configs.CUMULATIVE_LOAD_LIMIT * configs.CP_SAT_BW_FIRST_TH / 2.0):
                            u_load_rate = u_load_rate * configs.CP_SAT_ABOVE_BW_MULTP

                        if device_downlink_load[v.d_id] > (configs.CUMULATIVE_LOAD_LIMIT / 2.0):
                            v_load_rate = v_load_rate * 1000000000.0
                        elif device_downlink_load[v.d_id] > (configs.CUMULATIVE_LOAD_LIMIT * configs.CP_SAT_BW_FIRST_TH / 2.0):
                            v_load_rate = v_load_rate * configs.CP_SAT_ABOVE_BW_MULTP

                        max_delay:float = max(u_load_rate, v_load_rate)
                    else:
                        u_load_rate: float = u_rate * load / configs.CUMULATIVE_LOAD_LIMIT
                        v_load_rate: float = v_rate * load / configs.CUMULATIVE_LOAD_LIMIT
                    ...
                    if print_loads:
                        print(f"Load: {load}, u byte: {device_loads[u.d_id]}, v byte: {device_loads[v.d_id]}, u rate: {u_rate}, v rate: {v_rate}, u load rate: {u_load_rate}, v load rate: {v_load_rate}, max rate: {max_delay}, link load max: {link_delay_max}")

        if print_loads:
            print("=================================")

        return load, above_threshold, total_delay_sum, total_delay_min, total_delay_max, link_delay_sum, link_delay_max
    ...
    def get_topologically_ordered_nodes(self, reverse:bool=False) -> List[SpringNode]:
            ...
            if n.node_type == SpringElemenType.EVENTEXECUTION:
                ...
                if cep_node.cep_task.settings.action_name == event_name:
                    if n.d_id in event_devices:
                        raise Exception("A device can only have a single instance of an event!")
    ...
    def find_related_parents(self, topic_name:str) ->  List[SpringNode]:
        ...
        for n in self.G.nodes:
            if n.node_type in [SpringElemenType.HEAD, SpringElemenType.SINK, SpringElemenType.CONSUMER]:
                continue
    ...
    def find_related_children(self, topic_name:str) -> List[SpringNode]:
        ...
        n:SpringNode
        for n in self.G.nodes:
            if n.node_type in [SpringElemenType.HEAD, SpringElemenType.SINK, SpringElemenType.RAWOUTPUT]:
                continue
...
class RelaxationSolver:
    def __init__(self, 
        ...
        self.G = SpringGraph(workers, producers, event_consumers,
                    rawstats,
                    mssa,
                    consumerstats)
    ...
    def get_consumer_input_weight(self, consumer_name:str, consumer_topic:str, remote:bool=False) -> float:
        if not self.mopa_cost:
            return self.consumerstats.get_reading_byte_mul_ns_square(consumer_name, consumer_topic, remote)
        else:
            return self.consumerstats.get_reading_byte_mul_ns(consumer_name, consumer_topic, remote)

    def get_event_input_topic_weight(self, event_id:str, topic_id:str, remote:bool=False) -> float:
        if not self.mopa_cost:
            return self.mssa.topic_get_avg_byte_mul_ns_square(event_id, topic_id, remote)
        else:
    ...
    def generate_static_nodes(self):
        ...
        for _, raw_settings in self.producers.items():
            for _, prd in raw_settings.items():
                ...
                n_node = SpringNode(node_type=SpringElemenType.RAWOUTPUT, d_id=prd.producer_name, ele=new_source)
        ...
        for cons_d_id, cons_topics in self.consumers.items():
            for cons_topic_name, cons_topic_setting in cons_topics.items():
                ...
                n_node = SpringNode(node_type=SpringElemenType.CONSUMER, d_id=cons_topic_setting.host_name, ele=new_cons_node)
                self.G.add_static_node(n_node)
    ...
    def determine_initial_assignments(self, topology:Topology):
        ...
        print("Finding initial optimal nodes for the tasks with consideration of the static producers.")
        ...
        for on in ordered_task_nodes:
            ...
            input_topic_names:List[str] = [rt.input_topic for rt in cep_task.settings.required_sub_tasks]
            ...
            new_event_info = SpringEventElement(
                event_id=on.name,
                cep_task=cep_task
            )
            ...
            old_locations:List[str] = self.old_event_locations[cep_task.settings.action_name]
            ...
            for node, _, _ in node_relations:
                ...
                sp_node = SpringNode(node_type=SpringElemenType.EVENTEXECUTION, d_id=node.d_id, ele=new_event_info)
                assumed_weight = self.get_assumed_weight_for_theoretical_assignment(sp_node, node_relations, old_locations)
            ...
            for un_d_id in unrelated_nodes:
                ...
                sp_node = SpringNode(node_type=SpringElemenType.EVENTEXECUTION, d_id=un_d_id, ele=new_event_info)
                assumed_weight = self.get_assumed_weight_for_theoretical_assignment(sp_node, node_relations, old_locations)
                if assumed_weight == -1.0:
                    continue
                placement_suggestions[sp_node.d_id] = assumed_weight
            ...
            sorted_placements:dict[str, float] = dict(sorted(placement_suggestions.items(), key=lambda item: item[1]))
            ...
            print(f"number of sorted: {len(list(sorted_placements.keys()))}, {skip_load_check}")
            ...
            for d_id, _ in sorted_placements.items():
                ...
                if self.G.device_limit_reached(d_id, skip_load_check) or \
                    self.G.event_parallelization_limit_reached(new_event_info.cep_task.settings.action_name):
                    continue
                ...
                sp_node = SpringNode(node_type=SpringElemenType.EVENTEXECUTION, d_id=d_id, ele=new_event_info)
                ...
                for o_node, o_topic, is_parent in node_relations:
                    ...
                    if is_parent:
                        ...
                        topic_event_byte:float = self.mssa.get_expected_topic_load_per_second(cep_task.settings.action_name, o_topic, False)
                        if o_node.d_id == sp_node.d_id:
                            topic_event_byte = 0.0
                    
                        topic_cost = self.get_event_input_topic_weight(cep_task.settings.action_name, o_topic, o_node.d_id != sp_node.d_id)
                        
                        if sp_node.d_id not in old_locations and configs.RELAX_USE_OSCILLATION_PENALTY:
                            topic_event_byte = topic_event_byte * self.oscillation_penalty
                        
                        
                        weight = topic_cost * self.oscillation_penalty if sp_node.d_id not in old_locations else topic_cost
                        
                        self.G.add_edge(o_node, sp_node, weight=weight, topic=o_topic, expected_event_byte=topic_event_byte)
                        ...
                        if o_node.node_type == SpringElemenType.CONSUMER:
                            ...
                            topic_event_byte:float = self.consumerstats.get_expected_load_per_second(cons_ele.host_name, o_topic, False)
                            if o_node.d_id == sp_node.d_id:
                                topic_event_byte = 0.0                            
                            topic_cost = self.get_consumer_input_weight(o_node.d_id, o_topic, o_node.d_id != sp_node.d_id)
                        elif o_node.node_type == SpringElemenType.EVENTEXECUTION:
                            succ_ele:CEPTask = o_node.ele.cep_task
                            
                            topic_event_byte:float = self.mssa.get_expected_topic_load_per_second(succ_ele.settings.action_name, o_topic, False)
                            if o_node.d_id == sp_node.d_id:
                                topic_event_byte = 0.0

                            topic_cost = self.get_event_input_topic_weight(succ_ele.settings.action_name, o_topic, o_node.d_id != sp_node.d_id)
                        else:
                        ...
                        self.G.add_edge(sp_node, o_node, weight=weight, topic=o_topic, expected_event_byte=topic_event_byte)
    ...
    def get_assumed_weight_for_theoretical_assignment(self, 
                                         sp_node:SpringNode, 
                                         node_relations:List[Tuple[SpringNode, str, bool]],
                                         old_locations:List[str]) -> float:
        ...
        for o_node, o_topic, is_parent in node_relations:
            if is_parent:
                ...
                topic_event_byte:float = self.mssa.get_expected_topic_load_per_second(sp_node.ele.cep_task.settings.action_name, o_topic, False)
                if o_node.d_id == sp_node.d_id:
                    topic_event_byte = 0.0

                topic_cost = self.get_event_input_topic_weight(sp_node.ele.cep_task.settings.action_name, o_topic, o_node.d_id != sp_node.d_id)
    ...
    def update_predecessor_relationship(self, node:SpringNode, preds:List[SpringNode]):
        
        cep_node:CEPTask = node.ele.cep_task
        
        
        old_locations:List[str] = self.old_event_locations[cep_node.settings.action_name]

        
        for n in preds:
            
            topic_name:str = self.G.G[n][node]["topic"]
            topic_cost:float = self.get_event_input_topic_weight(cep_node.settings.action_name, topic_name, n.d_id != node.d_id)
            topic_event_byte:float = self.mssa.get_expected_topic_load_per_second(cep_node.settings.action_name, topic_name, False)
            if n.d_id == node.d_id:
                topic_event_byte = 0.0
            ...
            weight = topic_cost * self.oscillation_penalty if node.d_id not in old_locations else topic_cost
    ...
    def update_successor_relationship(self, node:SpringNode, succs:List[SpringNode]):
        ...
        for n in succs:
            ...
            if n.node_type == SpringElemenType.CONSUMER:
                ...
                topic_event_byte:float = self.consumerstats.get_expected_load_per_second(cons_ele.host_name, topic_name, False)
                if n.d_id == node.d_id:
                    topic_event_byte = 0.0

                topic_cost = self.get_consumer_input_weight(n.d_id, topic_name, n.d_id != node.d_id)
            elif n.node_type == SpringElemenType.EVENTEXECUTION:
                succ_ele:CEPTask = n.ele.cep_task
                
                topic_event_byte:float = self.mssa.get_expected_topic_load_per_second(succ_ele.settings.action_name, topic_name, False)
                if n.d_id == node.d_id:
                    topic_event_byte = 0.0
                
                topic_cost = self.get_event_input_topic_weight(succ_ele.settings.action_name, topic_name, n.d_id != node.d_id)
            else:
                raise Exception("Invalid node type detected!")
    ...
    def check_swapping(self, source_node:SpringNode, target_node:SpringNode, revert:bool) -> float:
    ...
    def iteration_solver_ease_springs(self):
        ...
        for spn in self.G.get_topologically_ordered_nodes(reverse=True):
            ...
            existing_event_locations = self.G.get_event_locations(see.cep_task.settings.action_name)

            
            for p in preds:
                
                if p.d_id == spn.d_id or p.d_id in force_comparer or p.d_id in existing_event_locations:
                    continue
                ...
                assumed_force = self.calculate_assumed_force(spn, preds, succs, p.d_id, old_id, revert=True)
            ...
            sorted_placements:dict[str, float] = dict(sorted(force_comparer.items(), key=lambda item: item[1]))
            ...
            if assigned_device != spn.d_id:
                ...
                old_id = deepcopy(spn.d_id)
                self.calculate_assumed_force(spn, preds, succs, assigned_device, old_id, revert=False)
    ...
    def calculate_assumed_force(self,
                                node:SpringNode, 
    ...
    def validate_assignment_dag(self, connection_validation_graph:SpringGraph):
        ...
        for u in connection_validation_graph.G.nodes:
            if not list(connection_validation_graph.G.successors(u)) and u != connection_validation_graph.Sink:
                connection_validation_graph.G.add_edge(u, connection_validation_graph.Sink)
                number_of_sink_edge_added += 1

        if number_of_sink_edge_added > 0:
            raise Exception("Sink connections should have been added with the static method!")
        ...
        if len(nx.bfs_tree(connection_validation_graph.G, connection_validation_graph.Head)) != connection_validation_graph.node_count:
            raise Exception(f"Not all nodes are between the source and the sink, number of nodes: {connection_validation_graph.node_count}, current number of connected nodes: {len(connection_validation_graph.G.nodes)}")
        ...
        if not is_directed or not is_acyclic or not is_weakly_connected:
            ...
            raise Exception("Graph needs to be directed acyclic and all nodes should connect to each other!")
        

    def determine_distribution(self) -> tuple[List[TaskAlterationModel], List[RawUpdateEvent], List[CEPConsumerUpdateEvent], dict[str, List[str]], float]:
        print("===============================")
        ...
        for spn in self.G.get_topologically_ordered_nodes():
            if spn.node_type == SpringElemenType.RAWOUTPUT:
                ...
                for succ in succs:
                    if succ.node_type == SpringElemenType.EVENTEXECUTION:
                        ...
                        if producer.output_topic.output_topic != sce.settings.topic_name:
                            raise Exception("Invalid topic connection exists in the graph!")
                ...
                if not producer.output_topic.target_databases:
                    raise Exception(f"Output targets cannot be empty:{producer.output_topic.output_topic}:{producer.producer_name}:{producer.raw_data_name}!")
                
                
                
                producer_updates.append(producer_target_db_update)
                if configs.LOGS_ACTIVE:
                    print(f"Assigning production: {producer.output_topic.output_topic} - {producer.raw_data_name}, to: {producer.producer_name}")
            ...
            if spn.node_type == SpringElemenType.EVENTEXECUTION:
                ...
                for succ in succs:
                    if succ.node_type == SpringElemenType.EVENTEXECUTION:
                        ...
                        see:SpringEventElement = succ.ele
                        for rt in see.cep_task.settings.required_sub_tasks:
                            if rt.input_topic == task.settings.output_topic.output_topic:
                                if topic_target not in rt.subscription_topics:
                                    rt.subscription_topics.append(topic_target)
                    elif succ.node_type == SpringElemenType.CONSUMER:
                        if configs.LOGS_ACTIVE:
                            print(f"Proessing consumer succesor: {succ.d_id}")
                        sce:SpringConsumerElement = succ.ele
                        if task.settings.output_topic.output_topic != sce.settings.topic_name:
                            raise Exception("Invalid topic connection exists in the graph!")
                        if topic_target not in sce.settings.source_topics:
                            sce.settings.source_topics.append(topic_target)
                ...
                if configs.LOGS_ACTIVE:
                    print(f"Assigning task: {task.settings.action_name}, to: {spn.d_id}")
            ...
            if spn.node_type == SpringElemenType.CONSUMER:
                ...
                if configs.LOGS_ACTIVE:
                    print(f"Assigning consumer: {cons_data.settings.host_name}:{cons_data.settings.topic_name}, from: {alteration.topics}")
        ...
        for td, activated_devices in task_activated_devices.items():
            if configs.LOGS_ACTIVE:
                print(f"activated devices: {activated_devices}, all devices: {self.G.worker_device_ids}")
        ...
        return alterations, producer_updates, consumer_alterations, new_distribution_history, max_load_perc
...
def relaxation(
        ...
        old_distribution_history:dict,    
        mopa_cost:bool=False
        ) -> tuple[List[TaskAlterationModel], List[RawUpdateEvent], List[CEPConsumerUpdateEvent], dict[str, List[str]], float]:
    print("----------------------------------------")
