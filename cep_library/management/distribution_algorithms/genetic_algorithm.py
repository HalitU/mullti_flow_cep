...
class CEPGeneticAlgorithm:
    def __init__(self, 
        ...
        self.event_consumers: dict[str, dict[str, CEPConsumerSettings]] = event_consumers
    ...
    def solve(self) -> tuple[List[TaskAlterationModel], List[RawUpdateEvent], List[CEPConsumerUpdateEvent], dict[str, EncodingModel], float]:
        ...
        self.population:List[Tuple[dict[str, EncodingModel], float, float, dict[str, float], float]] = self.initialize_population()
        ...
        print(f"Number of generations finished: {gen_ix}, best solution value: {best_citizen_in_population[1]}, bw load: {best_citizen_in_population[2]}")
    ...
    def update_best_solution(self, current_best_solution, population:List[Tuple[dict[str, EncodingModel], float, float, dict[str, float], float]]):
        sol_changed=False
        for p in population:            
    ...
    def ga_cross_single_point(self) -> List[Tuple[dict[str, EncodingModel], float, float, dict[str, float], float]]:
        ...
        for _ in range(0, self.population_size - self.parent_selection):
            ...
            first_encoding:dict[str, EncodingModel] = self.population[current_parents[0]][0]
            second_encoding:dict[str, EncodingModel] = self.population[current_parents[1]][0]
            ...
            for step_name, em in first_encoding.items():
                new_child_encoding[step_name] = EncodingModel(em.step_name, em.node_type)
                ...
                if em.node_type == NodeType.CONSUMER:
                    ...
                    new_child_encoding[step_name].register_output_topic(em.get_output_topic())
                ...
                if em.node_type == NodeType.RAWOUTPUT:                    
                    ...
                    if len(unique_set) > 1:
                        ...
                        new_output_targets = np.random.choice(a=unique_set, size=size_choice, replace=False)
                    else:
                    ...
                    if len(new_output_targets) == 0:
                        raise Exception(f"Number of output targets cannot be 0, {unique_set} - {valid_set}, {unique_valid_set}")
            ...
            child_fitness, bw_load, worker_external_loads, max_perc_load = self.calculate_fitness(new_child_encoding)

            if child_fitness == 0:
                raise Exception("Child fitness cannot be zero!")

            
            new_candidate = new_child_encoding, child_fitness, bw_load, worker_external_loads, max_perc_load
    ...
    def ga_mutate(self, parents_to_keep:List[Tuple[dict[str, EncodingModel], float, float, dict[str, float], float]], children:List[Tuple[dict[str, EncodingModel], float, float, dict[str, float], float]]):
        ...
        for p_ix in range((self.population_size - self.mutation_size), self.population_size):
            ...
            mutation_target, mutation_fitness, mutation_bw, worker_external_loads, max_perc_load = self.population[p_ix]
            ...
            for _, step_encoding in mutation_target.items():
                if step_encoding.node_type == NodeType.RAWOUTPUT:
                    ...
                    for ot_ix, ot_d in enumerate(output_targets):
                        prob_weights = [self.mutation_probability if w_l == ot_d else (1.0 - self.mutation_probability)/(len(worker_list)-1) for w_l in worker_list]
                    ...
                    crr_valid_worker_list = self.get_valid_workers_from_zero(worker_loads)
                    output_targets = step_encoding.get_execution_targets()
                    for ot_ix, ot_d in enumerate(output_targets):
                        coin_flip = np.random.choice(a=[1, 2], size=1, replace=False, p=[self.mutation_probability, (1.0 - self.mutation_probability)])
                    ...
                    if not configs.GENETIC_ALGO_WRITE_AT_DEVICE:
                        output_targets = step_encoding.get_output_targets()
                        for ot_ix, ot_d in enumerate(output_targets):
                            prob_weights = [self.mutation_probability if w_l == ot_d else (1.0 - self.mutation_probability)/(len(crr_valid_worker_list)-1) for w_l in crr_valid_worker_list]
                            if len(crr_valid_worker_list) == 1:
                                prob_weights=[1]
                            change = np.random.choice(a=crr_valid_worker_list, size=1, p=prob_weights)
                            step_encoding.mutate_output_target(ot_ix, change[0])                    
                            step_encoding.to_be_updated = True

            
            mutation_fitness, mutation_bw, worker_external_loads, max_perc_load = self.calculate_fitness(mutation_target)
            
            
            self.population[p_ix] = (mutation_target, mutation_fitness, mutation_bw, worker_external_loads, max_perc_load)
    ...
    def initialize_population(self):
        ...
        if self.last_best_encoding:
            w_val, bw_val, worker_external_loads,max_load_perc = self.calculate_fitness(self.last_best_encoding)
            last_enc = self.last_best_encoding, w_val, bw_val, worker_external_loads, max_load_perc
    ...
    def calculate_fitness(self, encoding:dict[str, EncodingModel]) -> Tuple[float, float, dict[str, float], float]:
        ...
        for raw_n in raw_nodes:
            ...
            self.traverse_raw_node(raw_n, ct, encoding, {}, {}, maximum_path_cost, worker_external_loads, worker_external_loads_keeper, worker_link_loads, worker_cpu_load)
        ...
        if configs.GA_USE_END_TO_END_LINK_MAX:
            max_load = 0.0
            tr_device_uplink_loads, tr_device_downlink_loads = self.get_device_load_penalties(worker_link_loads)            
            for raw_n in raw_nodes:
                self.traverse_raw_load(raw_n, link_ct, encoding, {}, {}, max_link_ct,
                                    tr_device_uplink_loads, tr_device_downlink_loads, worker_cpu_load)
        else:
            
            sum_load, max_load, link_load_sum, link_load_max = self.get_external_load_penalty(worker_external_loads, worker_link_loads, worker_cpu_load)
        ...
        device_uplink_load, device_downlink_load = self.get_device_load_penalties(worker_link_loads)
        max_load_perc = get_max_percentage(device_uplink_load, device_downlink_load)

        return (1.0 / maximum_path_cost.current_cost), max_load, worker_external_loads, max_load_perc
    ...
    def get_valid_workers(self, worker_cpu_usage:dict[str, int], worker_external_loads:dict[str, float], worker_cpu_load:dict[str, float], worker_link_loads:dict[str, dict[str, dict[str, dict[str, float]]]]):
        device_uplink_loads, device_downlink_loads = self.get_device_load_penalties(worker_link_loads)
        
        valid_workers:List[str] = []
        for k, v in worker_cpu_usage.items():
            limit_reached:bool = v > configs.DEVICE_ACTION_LIMIT + 1 or \
                (k in device_uplink_loads and configs.GA_USE_HARD_BW_LIMIT and device_uplink_loads[k] > (configs.CUMULATIVE_LOAD_LIMIT / 2.0)) or \
                (k in device_downlink_loads and configs.GA_USE_HARD_BW_LIMIT and device_downlink_loads[k] > (configs.CUMULATIVE_LOAD_LIMIT / 2.0)) or \
                worker_cpu_load[k] > configs.DEVICE_EXECUTION_LIMIT
        ...
        if len(valid_workers) == 0:
            ...
            for w_key, w_load in device_uplink_loads.items():
                sorted_placements[w_key] = max(w_load, sorted_placements[w_key])
            sorted_placements:dict[str, float] = dict(sorted(sorted_placements.items(), key=lambda item: item[1]))
    ...
    def traverse_event_node(self, 
        ...
        if not encoding_exists:
            ...
            valid_device_ids:List[str] = self.get_valid_workers(worker_cpu_usage, worker_external_loads, worker_cpu_load, worker_link_loads)
            valid_cpu_usages:dict[str, int] = {k:v for k,v in worker_cpu_usage.items() if k in valid_device_ids}
            
            probs = self.get_worker_probabilities(valid_cpu_usages)
            device_choices:List[str] = np.random.choice(valid_device_ids, size=device_count, p=probs, replace=False)
            ...
            device_choices:List[str] = np.random.choice(device_keys, size=device_count, p=probs, replace=False)
        ...
        new_step_encoding.register_output_topic(cep_task.settings.output_topic.output_topic)

        if self.last_best_encoding and node.name in self.last_best_encoding[node.name].get_output_targets():
            prev_output_target_hosts = self.last_best_encoding[node.name].get_output_targets()
        else:
        ...
        if configs.GA_COST_TYPE == 0:
            local_output_cost = self.mssa.event_output_get_avg_ns_over_byte(node.name, False)
            remote_output_cost = self.mssa.event_output_get_avg_ns_over_byte(node.name, True)
        elif configs.GA_COST_TYPE == 1:
            local_output_cost = self.mssa.event_output_get_avg_byte_mul_ns_square(node.name, False)
            remote_output_cost = self.mssa.event_output_get_avg_byte_mul_ns_square(node.name, True)        
        else:
        ...
        for rti in cep_task.settings.required_sub_tasks:
            if rti.input_topic == prev_output_topic:
                if configs.GA_COST_TYPE == 0:
                    remote_reading_cost += self.mssa.topic_get_avg_ns_over_byte(node.name, rti.input_topic, True)
                    local_reading_cost += self.mssa.topic_get_avg_ns_over_byte(node.name, rti.input_topic, False)
                elif configs.GA_COST_TYPE == 1:
                    remote_reading_cost += self.mssa.topic_get_avg_byte_mul_ns_square(node.name, rti.input_topic, True)
                    local_reading_cost += self.mssa.topic_get_avg_byte_mul_ns_square(node.name, rti.input_topic, False)        
                else:
                    raise Exception("Invalid GA_COST_TYPE")
                ...
                if execution_host != prev_output_host:
                    ...
                    if load_id not in worker_external_loads_keeper:
                        ...
                        if configs.GA_BANDWIDTH_COST_TYPE == 0:
                            ...
                            worker_external_loads[prev_output_host] += remote_reading_cost
                        elif configs.GA_BANDWIDTH_COST_TYPE == 1:
                            
                            estimated_input_load:float = self.mssa.get_expected_topic_load_per_second(cep_task.settings.action_name, rti.input_topic, False)
                            
                            if self.last_best_encoding:
                                previous_execution_locations:List[str] = self.last_best_encoding[node.name].get_execution_targets()
                                if configs.GA_USE_OSCILLATION_PENALTY and execution_host not in previous_execution_locations:
                                    estimated_input_load = estimated_input_load * configs.GA_OSCILLATION_PENALTY
                            
                            worker_external_loads[execution_host] += estimated_input_load
                            worker_external_loads[prev_output_host] += estimated_input_load
                            
                            
                            cpu_load:float = self.mssa.get_expected_event_input_counts(cep_task.settings.action_name, rti.input_topic)
                ...
                if (not configs.GA_EXLCUDE_EXECUTION_COST and execution_cost == 0) or local_reading_cost == 0 or remote_reading_cost == 0 or local_output_cost == 0 or remote_output_cost == 0:
        ...
        successors:List[TopologyNode] = self.topology.get_nodes_from_out_topic(cep_task.settings.output_topic.output_topic)

        for succ in successors:
            if succ.node_type == NodeType.EVENTEXECUTION:
                self.traverse_event_node(succ, ct, encodings, worker_data_usage, worker_cpu_usage, maximum_path_cost, node.name, worker_external_loads, worker_external_loads_keeper, worker_link_loads, worker_cpu_load)
            elif succ.node_type == NodeType.CONSUMER:
                self.traverse_consumer_node(succ, ct, encodings, maximum_path_cost, node.name, worker_external_loads, worker_external_loads_keeper, worker_link_loads, worker_cpu_load)

        
        ct.subtract_cost(total_step_cost)

    
    def traverse_raw_load(self, 
                        node:TopologyNode, 
                        ct:PathCostTracker, 
                        encodings:dict[str, EncodingModel], 
                        worker_data_usage:dict[str, int], 
        ...
        successors:List[TopologyNode] = self.topology.get_nodes_from_out_topic(data.output_topic.output_topic)
        
        for succ in successors:
            self.traverse_event_load(succ, ct, encodings, worker_data_usage, worker_cpu_usage, maximum_path_cost, node.name, device_uplink_load, device_downlink_load, worker_cpu_load)
        
    def traverse_event_load(self, 
                            node:TopologyNode, 
                            ct:PathCostTracker, 
                            encodings:dict[str, EncodingModel], 
                            worker_data_usage:dict[str, int], 
        ...
        for rti in cep_task.settings.required_sub_tasks:
            if rti.input_topic == prev_output_topic:
                if execution_host != prev_output_host:
                    u_device_load: float = device_uplink_load[prev_output_host] / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                    v_device_load: float = device_downlink_load[execution_host] / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                    
                    if device_uplink_load[prev_output_host] > (configs.CUMULATIVE_LOAD_LIMIT / 2.0):
                        u_device_load = u_device_load * 1000000.0
                    elif device_uplink_load[prev_output_host] > (configs.CUMULATIVE_LOAD_LIMIT * configs.CP_SAT_BW_FIRST_TH / 2.0):
                        u_device_load = u_device_load * configs.CP_SAT_ABOVE_BW_MULTP
                    if device_downlink_load[execution_host] > (configs.CUMULATIVE_LOAD_LIMIT / 2.0):
                        v_device_load = v_device_load * 1000000.0
                    elif device_downlink_load[execution_host] > (configs.CUMULATIVE_LOAD_LIMIT * configs.CP_SAT_BW_FIRST_TH / 2.0):
                        u_device_load = u_device_load * configs.CP_SAT_ABOVE_BW_MULTP
                        
                    if worker_cpu_load[prev_output_host] > configs.DEVICE_EXECUTION_LIMIT:
                        u_device_load = u_device_load * 1000000.0
        ...
        for succ in successors:
            if succ.node_type == NodeType.EVENTEXECUTION:
                ...
                self.traverse_consumer_load(succ, ct, encodings, maximum_path_cost, node.name, device_uplink_load, device_downlink_load, worker_cpu_load)
    
        
        ct.subtract_cost(step_cost)
    
    def traverse_consumer_load(self,
                            node:TopologyNode, 
                            ct:PathCostTracker, 
                            encodings:dict[str, EncodingModel], 
                            maximum_path_cost:PathCostTracker,
        ...
        for poh in prev_output_hosts:
            if poh != cons_data.host_name:
                u_device_load: float = device_uplink_load[poh] / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                v_device_load: float = device_downlink_load[cons_data.host_name] / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                
                if device_uplink_load[poh] > (configs.CUMULATIVE_LOAD_LIMIT / 2.0):
                    u_device_load = u_device_load * 1000000.0
                elif device_uplink_load[poh] > (configs.CUMULATIVE_LOAD_LIMIT * configs.CP_SAT_BW_FIRST_TH / 2.0):
                    u_device_load = u_device_load * configs.CP_SAT_ABOVE_BW_MULTP
                if device_downlink_load[cons_data.host_name] > (configs.CUMULATIVE_LOAD_LIMIT / 2.0):
                    v_device_load = v_device_load * 1000000.0
                elif device_downlink_load[cons_data.host_name] > (configs.CUMULATIVE_LOAD_LIMIT * configs.CP_SAT_BW_FIRST_TH / 2.0):
                    v_device_load = v_device_load * configs.CP_SAT_ABOVE_BW_MULTP
                    
                if worker_cpu_load[poh] > configs.DEVICE_EXECUTION_LIMIT:
                    u_device_load = u_device_load * 1000000.0
                    
                if worker_cpu_load[cons_data.host_name] > configs.DEVICE_EXECUTION_LIMIT:
                    v_device_load = v_device_load * 1000000.0
        ...
        maximum_path_cost.current_cost = ct.current_cost if ct.current_cost > maximum_path_cost.current_cost else maximum_path_cost.current_cost
        
        
        ct.subtract_cost(current_cost)

    def traverse_raw_node(self, 
                        node:TopologyNode, 
                        ct:PathCostTracker, 
                        encodings:dict[str, EncodingModel], 
                        worker_data_usage:dict[str, int], 
        ...
        if configs.GA_COST_TYPE == 0:
            raw_local_cost = self.rawstats.get_avg_ns_over_byte(data.producer_name, data.output_topic.output_topic, False)
            raw_remote_cost = self.rawstats.get_avg_ns_over_byte(data.producer_name, data.output_topic.output_topic, True)
        elif configs.GA_COST_TYPE == 1:
            raw_local_cost = self.rawstats.get_avg_byte_mul_ns_square(data.producer_name, data.output_topic.output_topic, False)
            raw_remote_cost = self.rawstats.get_avg_byte_mul_ns_square(data.producer_name, data.output_topic.output_topic, True)
        ...
        cpu_load = self.rawstats.get_expected_event_count(data.producer_name, data.output_topic.output_topic)
    ...
    def add_to_worker_link_loads(self, 
                                 worker_link_loads:dict[str, dict[str, dict[str, dict[str, float]]]], 
                                 event_name:str,
                                 topic_name:str,
                                 source:str, 
    ...
    def traverse_consumer_node(self,
                            node:TopologyNode, 
                            ct:PathCostTracker, 
                            encodings:dict[str, EncodingModel], 
                            maximum_path_cost:PathCostTracker,
        ...
        if configs.GA_COST_TYPE == 0:
            low_reading_cost = self.consumerstats.get_reading_ns_over_byte(cons_data.host_name, cons_data.topic_name, False)
            high_reading_cost = self.consumerstats.get_reading_ns_over_byte(cons_data.host_name, cons_data.topic_name, True)
        elif configs.GA_COST_TYPE == 1:
            low_reading_cost = self.consumerstats.get_reading_byte_mul_ns_square(cons_data.host_name, cons_data.topic_name, False)
            high_reading_cost = self.consumerstats.get_reading_byte_mul_ns_square(cons_data.host_name, cons_data.topic_name, True)
        else:
            raise Exception("Invalid GA_COST_TYPE")
            ...
            if poh != cons_data.host_name:
                ...
                if load_id not in worker_external_loads_keeper:
                    ...
                    if configs.GA_BANDWIDTH_COST_TYPE == 0:
                        ...
                        estimated_load:float = self.consumerstats.get_expected_load_per_second(cons_data.host_name, cons_data.topic_name, False)
                        worker_external_loads[cons_data.host_name] += estimated_load
                        worker_external_loads[poh] += estimated_load
                        
                        
                        cpu_load:float = self.consumerstats.get_expected_event_counts(cons_data.host_name, cons_data.topic_name)
    ...
    def get_device_load_penalties(self, worker_link_loads:dict[str, dict[str, dict[str, dict[str, float]]]]) -> Tuple[dict[str, float], dict[str, float]]:
        device_uplink_load:dict[str, float] = {}
        device_downlink_load:dict[str, float] = {}
    ...
    def get_external_load_penalty(self, 
            worker_external_loads:dict[str, float],
            worker_link_loads:dict[str, dict[str, dict[str, dict[str, float]]]],
            worker_cpu_load:dict[str, float]
        ...
        for _, event_topics in worker_link_loads.items():
            for _, topic_source_devices in event_topics.items():
                for dst_device, dst_devices in topic_source_devices.items():
                    for src_device, link_load_value in dst_devices.items():
                        u_load_rate: float = link_load_value / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                        v_load_rate: float = link_load_value / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                        
                        u_device_load: float = device_uplink_load[src_device] / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                        v_device_load: float = device_downlink_load[dst_device] / (configs.CUMULATIVE_LOAD_LIMIT / 2.0)
                        ...
                        if configs.GA_USE_UP_DOWN_LINK_COST_ADVANCED:
                            ...
                            if device_uplink_load[src_device] > (configs.CUMULATIVE_LOAD_LIMIT / 2.0):
                                u_load_rate = u_load_rate * 1000000.0
                            elif device_uplink_load[src_device] > (configs.CUMULATIVE_LOAD_LIMIT * configs.CP_SAT_BW_FIRST_TH / 2.0):
                                u_load_rate = u_load_rate * configs.CP_SAT_ABOVE_BW_MULTP
                            if device_downlink_load[dst_device] > (configs.CUMULATIVE_LOAD_LIMIT / 2.0):
                                v_load_rate = v_load_rate * 1000000.0
                            elif device_downlink_load[dst_device] > (configs.CUMULATIVE_LOAD_LIMIT * configs.CP_SAT_BW_FIRST_TH / 2.0):
    ...
    def generate_citizen(self) -> Tuple[dict[str, EncodingModel], float, float, dict[str, float], float]:
        ...
        for raw_n in raw_nodes:
            ...
            self.traverse_raw_node(raw_n, ct, step_encodings, worker_data_usage, worker_cpu_usage, maximum_path_cost, worker_external_loads, worker_external_loads_keeper, worker_link_loads, worker_cpu_load)
        ...
        if configs.GA_USE_END_TO_END_LINK_MAX:
            ...
            tr_device_uplink_loads, tr_device_downlink_loads = self.get_device_load_penalties(worker_link_loads)
            for raw_n in raw_nodes:
                self.traverse_raw_load(raw_n, link_ct, step_encodings, {}, {}, max_link_ct, 
                                    tr_device_uplink_loads, tr_device_downlink_loads, worker_cpu_load)
        else:
        ...
        return step_encodings, (1.0 / maximum_path_cost.current_cost), max_load, worker_external_loads, max_load_perc

    
    def get_final_distribution(self, encoding) -> tuple[List[TaskAlterationModel], List[RawUpdateEvent],
                                                        List[CEPConsumerUpdateEvent], dict[str, EncodingModel], float]:
        
        step_targets:dict[str, EncodingModel]
        total_cost:float
        step_targets, total_cost, bw_cost, worker_external_loads, max_load_perc = encoding
        ...
        for step in self.topology.get_topological_ordered_nodes():
            ...
            if step.node_type == NodeType.RAWOUTPUT:
                ...
                print(f"Raw source {step.name} will write: {producer.output_topic.output_topic} its output to: {current_step_targets}")
            ...
            if step.node_type == NodeType.EVENTEXECUTION:
                ...
                print(f"Event {step.name} will be executed at : {execution_targets}, will write its outputs to: {event_output_targets}")
                ...
                for exec in execution_targets:
                    ...
                    if number_of_reads == 0:
                        raise Exception("Execution cannot have 0 input sources assigned to it!")
                    ...
                    for c in event_output_targets:
                        if c not in topic_targets[task.settings.output_topic.output_topic]:
                            topic_targets[task.settings.output_topic.output_topic].append(c)
                ...
                for host in [*workers]:
                    if host not in execution_targets:
                        ...
                        migration_request.topic = task.settings.output_topic.output_topic
            ...
            if step.node_type == NodeType.CONSUMER:
                ...
                print(f"Assigning consumer: {cons_et.host_name}:{cons_et.topic_name}, to: {alteration.topics}")
        ...
        return alterations, producer_updates, consumer_alterations, step_targets, max_load_perc
...
def genetic_algorithm(
        ...
        consumerstats:ConsumerServerStatics,
        last_best_encoding:dict
) -> tuple[List[TaskAlterationModel], List[RawUpdateEvent], List[CEPConsumerUpdateEvent], dict[str, EncodingModel]]:
