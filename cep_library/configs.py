...
env_mongo_self:str = str(os.getenv('MONGO_SELF_HOST')) if os.getenv('MONGO_SELF_HOST') is not None else "mongo"
env_mongo_host:str = str(os.getenv('MONGO_ONE_HOST')) if os.getenv('MONGO_ONE_HOST') is not None else "127.0.0.1"
env_mongo_port:str = str(os.getenv('MONGO_ONE_PORT')) if os.getenv('MONGO_ONE_PORT') is not None else "27017"
data_carry_enabled = bool(os.getenv('DATA_CARRY_ENABLED')) if os.getenv('DATA_CARRY_ENABLED') is not None else False
data_operation_timeout:int = int(str(os.getenv('DATA_OPERATION_TIMEOUT'))) if os.getenv('DATA_OPERATION_TIMEOUT') is not None else 30
data_max_connection = int(str(os.getenv('DATA_MAX_CONNECTION'))) if os.getenv('DATA_MAX_CONNECTION') is not None else 30
collection_lifetime = int(str(os.getenv('COLLECTION_LIFETIME'))) if os.getenv('COLLECTION_LIFETIME') is not None else 60
data_carry_window = int(str(os.getenv('DATA_CARRY_WINDOW'))) if os.getenv('DATA_CARRY_WINDOW') is not None else 30

server_query_type = int(str(os.getenv('SERVER_QUERY_TYPE'))) if os.getenv('SERVER_QUERY_TYPE') is not None else 1
...
CP_RAM_LIMIT = float(str(os.getenv('CP_RAM_LIMIT'))) if os.getenv('CP_RAM_LIMIT') is not None else 5000000.0

print(f"Optimizer RAM limit: {CP_RAM_LIMIT}")

simulation_duration = int(str(os.getenv('SIM_DURATION'))) if os.getenv('SIM_DURATION') is not None else 300

query_with_timedelta = int(str(os.getenv('QUERY_WITHIN_TIMEDELTA'))) if os.getenv('QUERY_WITHIN_TIMEDELTA') is not None else 1000

print(f"Simulation raw producer settings; sim duration: {simulation_duration}, queries are within timedelta: {query_with_timedelta}")

env_server_name:str = str(os.getenv('SERVER_NAME')) if os.getenv('SERVER_NAME') is not None else "127.0.0.1"
env_server_port = int(str(os.getenv('SERVER_PORT'))) if os.getenv('SERVER_PORT') is not None else 8000

env_distribution_type = int(str(os.getenv('DISTRIBUTION_TYPE'))) if os.getenv('DISTRIBUTION_TYPE') is not None else 0
print("Env server name is: ", os.getenv('SERVER_NAME'))

env_host_name:str = str(os.getenv('HOST_NAME')) if os.getenv('HOST_NAME') is not None else "client_one"
env_host_ip:str = str(os.getenv('HOST_IP')) if os.getenv('HOST_IP') is not None else "127.0.0.1"
env_host_port = int(str(os.getenv('HOST_PORT'))) if os.getenv('HOST_PORT') is not None else 8080
env_host_x:int = int(str(os.getenv('HOST_X'))) if os.getenv('HOST_X') is not None else 100
env_host_y:int = int(str(os.getenv('HOST_Y'))) if os.getenv('HOST_Y') is not None else 100

evaluation_period = int(str(os.getenv('EVAL_PERIOD_SECS'))) if os.getenv('EVAL_PERIOD_SECS') is not None else 30
...
IMAGE_FETCH_LIMIT = int(str(os.getenv('IMAGE_FETCH_LIMIT'))) if os.getenv('IMAGE_FETCH_LIMIT') is not None else 1

USE_HISTORIC_DIST_AFTER_N = int(str(os.getenv('USE_HISTORIC_DIST_AFTER_N'))) if os.getenv('USE_HISTORIC_DIST_AFTER_N') is not None else 0

CP_RUNTIME_SECONDS = int(str(os.getenv('CP_RUNTIME_SECONDS'))) if os.getenv('CP_RUNTIME_SECONDS') is not None else 2

MQTT_HOST:str = str(os.getenv('MQTT_HOST')) if os.getenv('MQTT_HOST') is not None else 'mqttserver'
MQTT_PORT:int = int(str(os.getenv('MQTT_PORT'))) if os.getenv('MQTT_PORT') is not None else 1883

EVAL_ACTIVE:int = int(str(os.getenv('EVAL_ACTIVE'))) if os.getenv('EVAL_ACTIVE') is not None else 0


DISABLE_VSM:int = int(str(os.getenv('DISABLE_VSM'))) if os.getenv('DISABLE_VSM') is not None else 0

print(f"Is VSM activated: {DISABLE_VSM == 0}")

MIN_COST_AFTER_N_DIST:int = int(str(os.getenv('MIN_COST_AFTER_N_DIST'))) if os.getenv('MIN_COST_AFTER_N_DIST') is not None else 2
ENFORCE_PRODUCTION_LOCATION:int = get_int_from_env('ENFORCE_PRODUCTION_LOCATION', 0)
DIFFERENT_DEVICE_READ_PENATLY:float = get_float_from_env('DIFFERENT_DEVICE_READ_PENATLY', 1.5)
...
STAT_NORMALIZER_LOW:float = float(str(os.getenv('STAT_NORMALIZER_LOW'))) if os.getenv('STAT_NORMALIZER_LOW') is not None else 10.0
STAT_NORMALIZER_HIGH:float = float(str(os.getenv('STAT_NORMALIZER_HIGH'))) if os.getenv('STAT_NORMALIZER_HIGH') is not None else 100.0

ALARM_BROADCAST_DURATION:float = float(str(os.getenv('ALARM_BROADCAST_DURATION'))) if os.getenv('ALARM_BROADCAST_DURATION') is not None else 0.25


CEP_SAT_OPT_FUNCTION:int = int(str(os.getenv('CEP_SAT_OPT_FUNCTION'))) if os.getenv('CEP_SAT_OPT_FUNCTION') is not None else 1
EVENT_ACTIVATION_MULTIPLIER = float(str(os.getenv('CP_SAT_EVENT_ACTIVATION_MULTIPLIER'))) if os.getenv('CP_SAT_EVENT_ACTIVATION_MULTIPLIER') is not None else 1.2
CEP_SAT_BANDWIDTH_BALANCER:int = int(str(os.getenv('CEP_SAT_BANDWIDTH_BALANCER'))) if os.getenv('CEP_SAT_BANDWIDTH_BALANCER') is not None else 1

STATISTICS_REMOTE_MULTIPLIER:float = float(str(os.getenv('STATISTICS_REMOTE_MULTIPLIER'))) if os.getenv('STATISTICS_REMOTE_MULTIPLIER') is not None else 1.2

ALLOWED_PARALLEL_EXECUTION_COUNT:int = int(str(os.getenv('ALLOWED_PARALLEL_EXECUTION_COUNT'))) if os.getenv('ALLOWED_PARALLEL_EXECUTION_COUNT') is not None else 2


IS_LOCAL:int = int(str(os.getenv('IS_LOCAL'))) if os.getenv('IS_LOCAL') is not None else 0

scenario_case:int = 2
ALLOWED_PARALLEL_EXECUTION_COUNT=1
...
data_pattern_json_file="vanet_apps/data/dataconfigs/sensor_data_pattern_less_load.json"
USE_MQTT_FOR_DATA_TRANSFER:bool=False 
MQTT_QOS_LEVEL=2
MQTT_CONSUMER_QOS_LEVEL=2
CUMULATIVE_LOAD_LIMIT:float=2 * 125 * 4000 
