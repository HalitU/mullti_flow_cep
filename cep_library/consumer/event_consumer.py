...
class CEPEventConsumer:
    def __init__(self, 
                 ...
                 db:DatabaseManagementService) -> None:
        ...
        self.consumer_stats = ConsumerTopicStats(self.settings.host_name, self.settings.topic_name)
    ...
    def register_action_file_service(self, app: FastAPI):
        ...
        def event_consumerraw_statistics_report():
            ...
            collected_data_count = usage.local_read_data_count.count + usage.remote_read_data_count.count
            
            print(f"==========================================================================================")
            print(f"[CONSUMER] {configs.env_host_name}:{self.settings.topic_name} miss count: {usage.miss_count.count}, avg read per miss: {usage.miss_count.mean} ")
            print(f"[CONSUMER] {configs.env_host_name}:{self.settings.topic_name} hit count: {usage.hit_count.count}, avg read per miss: {usage.hit_count.mean} ")
            for raw_hc_name, raw_hc_count in usage.source_hit_count.items():
                print(f"[CONSUMER] {configs.env_host_name}:{self.settings.topic_name}:{raw_hc_name} hit count: {raw_hc_count.count}, avg read per miss: {usage.source_miss_count[raw_hc_name].count} ")
    ...
    def send_heartbeat(self):
        ...
        server_url = 'http://'+configs.env_server_name+':'+str(configs.env_server_port)+'/event_consumer/'
    ...
    def trigger_alteration(self, msg: bytes, _):
        ...
        if target_update_request.host != self.settings.host_name or target_update_request.topic_name != self.settings.topic_name:
    ...
    def get_data_from_vsm(self, em_request:EventModel):
        ...
        event_received_duration:float = (datetime.datetime.now(datetime.timezone.utc) - em_request.event_date).microseconds / 1000.0
        ...
        if em_request.vsm == self.settings.host_name:
            ...
            in_local_read_duration += data_helper.get_elapsed_ms(crr_read_time_start) + event_received_duration
            in_local_data_count += len(current_queried_data)
            in_local_count += 1
        else:
            in_remote_data_byte += crr_data_size
            in_remote_read_duration += data_helper.get_elapsed_ms(crr_read_time_start) + event_received_duration
        ...
        if len(current_queried_data) > 0:
            ...
            if em_request.raw_data_tracker not in self.consumer_stats.source_hit_count:
                self.consumer_stats.source_hit_count[em_request.raw_data_tracker] = MeanStatBase()
            if em_request.raw_data_tracker not in self.consumer_stats.source_miss_count:
                self.consumer_stats.source_miss_count[em_request.raw_data_tracker] = MeanStatBase()
