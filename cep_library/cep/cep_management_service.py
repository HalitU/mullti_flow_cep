...
class CEPEventStatisticsManager:
    ...
    def reset_action_statistics(self):
        ...
        print(
            f"[CLIENT] {self.cepsm.host_name} Deleted {len(action_stats_to_remove)} leftover stats."
        )
    ...
    def resource_usage(self) -> bytes:
        ...
        for action_name, action_statistic in self.action_statistics.items():
            ...
            crr_execution_count += int(
                action_statistic.crr_task_execution_ns_stats.count
            )

            print(
                f"[CLIENT] action: {action_name} hit count: {action_statistic.hit_count.count}, miss count: {action_statistic.miss_count.count}"
            )
        print("----------------------------------------------------")
...
class CEPEventActionManager:
    def __init__(
        self, cepsm: CEPEventStateManager, cep_stats_manager: CEPEventStatisticsManager
    ) -> None:
        self.cepsm = cepsm
        ...
        try:
            ...
            if len(current_queried_data) > 0:
                event_received_duration: float = (
                    datetime.now(timezone.utc) - em_request.event_date
                ).microseconds / 1000.0
                ...
                if em_request.vsm == host_name:
                    tsd.local_ns += (
                        data_helper.get_elapsed_ms(
                            crr_db_current_in_topic_read_start_time
                        )
                        + event_received_duration
                    )
                    tsd.local_byte += data_helper.estimate_size_kbytes(
                        current_queried_data
                    )
                    ...
                    tsd.remote_byte += data_helper.estimate_size_kbytes(
                        current_queried_data
                    )
                ...
                print(
                    f"Could not find data at vsm location: {em_request.vsm}, for topic: {triggered_topic_name}: for action: {action.settings.action_name}, with data id: {em_request.data_id}"
                )
            ...
            if custom_args:
                (result, trigger_next, _, data_priority) = action.task(
                    data, custom_args
                )
            else:
            ...
            if trigger_next:
                ...
                target_hosts: List[str] = deepcopy(
                    action.settings.output_topic.target_hosts
                )
                target_db_list: List[str] = deepcopy(
                    action.settings.output_topic.target_databases
                )
                for target_db in target_db_list:
                    ...
                    if configs.MQTT_USE_PREFIX_FOR_EVENTS:
                        ...
                        for th in target_hosts:
                            fixed_output_topic: str = (
                                f"{action.settings.output_topic.output_topic}_{th}"
                            )
                            action_producer.produce(fixed_output_topic, payload)
                    else:
                        fixed_output_topic: str = (
                            action.settings.output_topic.output_topic
                        )
                    ...
                    event_received_duration: float = (
                        datetime.now(timezone.utc) - core_data_time
                    ).microseconds / 1000.0
                    event_received_duration = (
                        10.0 if event_received_duration < 0 else event_received_duration
                    )
                    ...
                    if target_db == host_name:
                        ...
                        out_local_elapsed_ms += (
                            data_helper.get_elapsed_ms(crr_db_data_write_start_time)
                            + event_received_duration
                        )
                        out_local_data_count += 1
                    else:
                        out_remote_data_byte += out_data_byte
                        out_remote_elapsed_ms += (
                            data_helper.get_elapsed_ms(crr_db_data_write_start_time)
                            + event_received_duration
                        )
            ...
            raw_elapsed_max_time_from_init_ms = (
                crr_event_date - min(initDates)
            ).total_seconds() * 1000.0
            raw_elapsed_min_time_from_init_ms = (
                crr_event_date - max(initDates)
            ).total_seconds() * 1000.0
            raw_elapsed_time_from_init_ms = raw_elapsed_time_from_init_ms / len(
                initDates
            )
            ...
            sst.action_name = action.settings.action_name
            sst.total_elapsed = (
                default_clock_sync_delay_s if total_elapsed < 0 else total_elapsed
            )
            sst.execution_elapsed_ms = execution_elapsed
            sst.task_execution_time = task_execution_time

            sst.raw_elapsed_time_from_init_ms = (
                default_clock_sync_delay_ms
                if raw_elapsed_time_from_init_ms < 0
                else raw_elapsed_time_from_init_ms
            )
            sst.raw_elapsed_max_time_from_init_ms = (
                default_clock_sync_delay_ms
                if raw_elapsed_max_time_from_init_ms < 0
                else raw_elapsed_max_time_from_init_ms
            )
            sst.raw_elapsed_min_time_from_init_ms = (
                default_clock_sync_delay_ms
                if raw_elapsed_min_time_from_init_ms < 0
                else raw_elapsed_min_time_from_init_ms
            )
            ...
            if configs.DEBUG_MODE:
                print(
                    "[ERROR] Probably action is deactivated before it can be finished: ",
                    e,
                    " ",
...
class CEPEventCodeManager:
    def __init__(
        ...
        cep_action_manager: CEPEventActionManager,
    ) -> None:
        self.cepsm = cepsm
        self.cep_stats_manager = cep_stats_manager
    ...
    def download_script_file(self, action_path) -> requests.Response:
        server_url = (
            ...
            + "/get_server_action_file/"
        )

        try:
            params = {"file_path": action_path}
    ...
    def add_action(self, action: CEPTask):
        ...
        if action.settings.action_name in self.cepsm.actions:
            print(
                f"[CLIENT] {self.cepsm.host_name} Action {action.settings.action_name} is already active, possibly periodic evaluation is triggered manually!"
            )
            return

        add_action_start = data_helper.get_start_time()

        downloaded_module_file: requests.Response = self.download_script_file(
            action.settings.action_path
        )
        ...
        if action.settings.action_name in self.cep_stats_manager.action_statistics:
            print(
                f"No stats should be availabe for the newly activated action: {action.settings.action_name}!"
            )
            sys.exit()

        statistic_record = StatisticRecord(action.settings.action_name)
        for t in action.settings.required_sub_tasks:
            statistic_record.in_topic_stats[t.input_topic] = TopicStatisticRecord(
                t.input_topic
            )
        self.cep_stats_manager.action_statistics[action.settings.action_name] = (
            statistic_record
        )
        ...
        if configs.MQTT_SHARED_EVENT_CLIENT_ID:
            publisher_client_id: str = (
                f"producer_{action.settings.action_name}_{action.settings.output_topic.output_topic}"
            )
        else:
            publisher_client_id: str = (
                f"producer_{action.settings.action_name}_{action.settings.output_topic.output_topic}_{self.cepsm.host_name}"
            )
        action_producer = MQTTProducer(
            client_id=publisher_client_id, host_name=self.cepsm.host_name
        )
        ...
        for required_sub_task in action.settings.required_sub_tasks:
            ...
            if configs.MQTT_SHARED_EVENT_CLIENT_ID:
                ...
                crr_client_id: str = (
                    f"{action.settings.action_name}_{topic_name}_{self.cepsm.host_name}"
                )
            ...
            action_consumer = MQTTConsumer(
                ...
                target_args=(
                    ...
                    stat_lock,
                    statistic_record,
                    self.cepsm.host_name,
                    action_producer,
                    db_query,
                    topic_name,
                    action.settings.arguments,
                ),
        ...
        print(
            f"[!!!] {self.cepsm.host_name} worker activated action: ",
            action.settings.action_name,
            " ",
            imported_method,
            ", in: ",
    ...
    def remove_action(self, action_name: str, deletion_time: datetime):
        print("---------------------------------------------")
        print(
            f"[!!] [CEP EXECUTOR] {self.cepsm.host_name} Removing action: ", action_name
        )
        ...
        for consumer in action["consumer"]:
            ...
            while consumer.last_msg_date + timedelta(seconds=5) >= datetime.now(
                timezone.utc
            ):
        ...
        if configs.DEBUG_MODE:
            print(
                "Pre-validation of module existence: ",
                action["module_name"],
                flush=True,
            )
    ...
    def process_consumer_alteration(self, msg: bytes, args):
        ...
        if (
            ...
            for activation in alteration_request.activations:
                ...
                if (
                    activation.job_name != ""
                    and activation.job_name not in self.cepsm.actions
                ):
                    ...
                    activation.job_name != ""
                    and activation.job_name in self.cepsm.actions
                ):
                    self.cepsm.actions[activation.job_name]["last_update_date"] = (
                        datetime.now(timezone.utc)
                    )

                    activated_action_to_manipulate: CEPTask = self.cepsm.actions[
                        activation.job_name
                    ]["action"]
                    ...
                    new_output_dbs += (
                        activation.cep_task.settings.output_topic.target_databases
                    )

                    old_output_dbs: List[str] = []
                    old_output_dbs += (
                        activated_action_to_manipulate.settings.output_topic.target_databases
                    )

                    common_dbs = list(set(new_output_dbs).intersection(old_output_dbs))

                    if len(new_output_dbs) != len(old_output_dbs) or len(
                        common_dbs
                    ) != len(old_output_dbs):
                        print(
                            f"[CLIENT] {self.cepsm.host_name} Output target difference detected!"
                        )

                    for (
                        old_req
                    ) in activated_action_to_manipulate.settings.required_sub_tasks:
                        new_req = [
                            s
                            for s in activation.cep_task.settings.required_sub_tasks
                            if s.input_topic == old_req.input_topic
                        ][0]
                        ...
                        common_dbs = list(
                            set(new_output_dbs).intersection(old_output_dbs)
                        )
                        ...
                        if len(new_output_dbs) != len(old_output_dbs) or len(
                            ...
                            print(
                                f"[CLIENT] {self.cepsm.host_name} Input target difference detected!"
                            )

                    for (
                        new_target_db
                    ) in activation.cep_task.settings.output_topic.target_databases:
                    ...
                    activated_action_to_manipulate.settings = (
                        activation.cep_task.settings
                    )

            for deactivation in alteration_request.deactivations:
                if (
                    deactivation.job_name != ""
                    and deactivation.job_name in self.cepsm.actions
                ):
                    print(f"Triggering removal of action: {deactivation.job_name}")
                    Thread(
                        daemon=True,
                        target=self.remove_action,
                        args=(
                            deactivation.job_name,
                            datetime.now(timezone.utc),
                        ),
...
class CEPEventHTTPEndpointManager:
    def __init__(
        ...
        cep_event_statistics_manager: CEPEventStatisticsManager,
    ) -> None:
        self.cepsm = cepsm
...
class CEPEventMessageCommunicationManager:
    def __init__(
        ...
        cepsm: CEPEventStateManager,
        cep_stats_manager: CEPEventStatisticsManager,
        cep_code_manager: CEPEventCodeManager,
    ) -> None:
        self.cepsm = cepsm
        self.cep_stats_manager = cep_stats_manager
    ...
    def setup_communication(self):
        ...
        alteration_topic: str = (
            f"{configs.kafka_alteration_topics[0]}_{self.cepsm.host_name}"
        )
...
class CEPManagementService:
    def __init__(
        self, host_name: str, app: FastAPI, db: DatabaseManagementService
    ) -> None:
        ...
        self.cepeem = CEPEventHTTPEndpointManager(
            self.cepsm, self.stop_management, self.esm
        )
        ...
        self.cepemcm = CEPEventMessageCommunicationManager(
            self.cepsm, self.esm, self.ecm
        )
