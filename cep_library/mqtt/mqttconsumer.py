...
class MQTTConsumer:
    def __init__(
        ...
        if configs.MQTT_LOG_ACTIVE:
            print(
                f"[MQTTConsumer] Consumer connecting to {configs.MQTT_HOST}, {configs.MQTT_PORT}, with client id: {self.host_name}:{self.client_id}"
            )
        ...
        while True:
            try:
                self.client.connect(
                    ...
                    configs.MQTT_PORT,
                    60,
                    clean_start=configs.MQTT_CLEAN_START,
                )
                break
            except Exception as _:
                print(
                    f"{self.host_name}:{self.client_id} Waiting to connect to MQTT broker..."
                )
                pass
            time.sleep(1.0)

    def on_connect(
        self, client: mqtt_client.Client, userdata, flags, reason_code, properties
    ):
        if reason_code == 0:
        ...
        if reason_code > 0:
            print(
                f"[MQTTConsumer] {self.host_name}:{self.client_id} Error with result code "
                + str(reason_code)
            )
        ...
        for t in self.topics:
            ...
            topic_tuple.append(
                (
                    s_t,
                    SubscribeOptions(qos=configs.MQTT_CONSUMER_QOS_LEVEL, noLocal=True),
                )
            )
    ...
    def on_disconnect(self, client, userdata, reason_code, properties):
        if configs.MQTT_LOG_ACTIVE:
            print(
                f"[MQTTConsumer] {self.host_name}:{self.client_id} Client disconnected..."
            )

    def on_subscribe(self, client, userdata, mid, reason_codes, properties):
        print(
            f"[MQTTConsumer] {self.host_name}:{self.client_id} Subscribed: "
            + str(mid)
            + " "
            + str(reason_codes)
        )
    ...
    def on_unsubscribe(self, client, userdata, mid, reason_code_list, properties):
        ...
        print(
            f"[MQTTConsumer] {self.host_name}:{self.client_id} un-subscribed: "
            + str(mid)
            + " "
            + str(reason_code_list)
        )
    ...
    def run(self):
        if configs.MQTT_LOG_ACTIVE:
            print(
                f"[MQTTConsumer] {self.host_name}:{self.client_id} Running the blocking loop..."
            )
        try:
            self.client.loop_forever(
                retry_first_connection=configs.MQTT_RECONNECT_FIRST_TRY
            )
            ...
            if "BulkWriteError" == type(e).__name__:
                ...
                print(
                    f"{self.host_name}:{self.client_id} Unexpected error occured during loop: ",
                    str(e),
                )
        if configs.MQTT_LOG_ACTIVE:
            print(
                f"[MQTTConsumer] Consumer {self.host_name}:{self.client_id} timeout or disconnection happened..."
            )
