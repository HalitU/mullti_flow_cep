...
class AmbulanceAlarmConsumer:
    def __init__(self, app:FastAPI, db:DatabaseManagementService) -> None:
        self.topic_name = "event.vehicle_type_alarm_task"
    ...
    def broadcast(self, event_data: dict[str, datetime | List[Any] | Any], raw_data_tracker:str) -> None:
        if "vehicle_type_alarm_task" not in event_data or len(event_data["vehicle_type_alarm_task"]) == 0:
        ...
        if "vehicle_type_alarm_task" in event_data:
            # Dates
            ...
            for aa in event_data["vehicle_type_alarm_task"]:
