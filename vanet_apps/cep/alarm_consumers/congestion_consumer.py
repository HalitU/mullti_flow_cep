...
class CongestionConsumer:
    def __init__(self, app:FastAPI, db:DatabaseManagementService) -> None:
        self.topic_name = "event.traffic_congestion_task"
        ...
        query: MongoQuery = self.get_mongo_query()
        ...
        print("Initialized the congestion alarm consumer.")

    def get_mongo_query(self, read_timedelta:int = configs.query_with_timedelta, limit:int = configs.MIN_REQUIRED_ACTIVE_VAR) -> MongoQuery:
    ...
    def broadcast(self, event_data: dict[str, datetime | List[Any] | Any], raw_data_tracker:str) -> None:
        if "traffic_congestion_task" not in event_data or len(event_data["traffic_congestion_task"]) == 0:
        ...
        if "traffic_congestion_task" in event_data:
            # Dates
            ...
            for aa in event_data["traffic_congestion_task"]:
                # Write the time it took to process a data to a statistics file
                data_exists = True
                alarm_exists: bool = aa["alarm_exists"]
            raw_processing_duration: float = (datetime.now(tz=timezone.utc) - oldest_date).total_seconds()
            latest_processing_duration: float = (datetime.now(tz=timezone.utc) - newest_data).total_seconds()
            ...
            if data_exists:
                ...
                for src_name, src_stat in self.cec.consumer_stats.source_hit_count.items():
                    ...
                    miss_cnt: MeanStatBase = self.cec.consumer_stats.source_miss_count[src_name]
