...
class CivilianDensityConsumer:
    def __init__(self, app:FastAPI, db:DatabaseManagementService) -> None:
        self.topic_name = "event.civilian_density_task"
        self.db = db
    ...
    def get_mongo_query(self, read_timedelta = configs.query_with_timedelta, limit:int = configs.MIN_REQUIRED_ACTIVE_VAR):
        print(f"[CLIENT] setting read timedelta as: {read_timedelta}")
    ...
    def broadcast(self, event_data: dict, raw_data_tracker:str):
        ...
        if "civilian_density_task" not in event_data or len(event_data["civilian_density_task"]) == 0:
        ...
        if "civilian_density_task" in event_data:
            ...
            for aa in event_data["civilian_density_task"]:
