...
class TemperatureDataSensor(BaseDataSensor):
    def __init__(self, 
        ...
        print(f"[TemperatureDataSensor] Initializing with params: {sdp.msg_per_delay}, {sdp.default_delay_s}")
    ...
    def run(self):    
        ...
        while not stop:
            ...
            for _ in range(count_per_delay):
                ...
                data = dict()
                data[self.settings.raw_data_name] = [(temp_min - temp_max) * np.random.random_sample() + temp_max]
                ...
                self.producer.send(data, raw_data_tracker="temperature_data")
                
                msg_count += 1
                crr_sleep_duration: float = (delayS * 1.0) / count_per_delay
