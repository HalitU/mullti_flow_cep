...
class NoiseSensor(BaseDataSensor):
    def __init__(self, 
        ...
        print(f"[NoiseDataSensor] Initializing with params: {sdp.msg_per_delay}, {sdp.default_delay_s}")
    ...
    def run(self):    
        ...
        while not stop:
            ...
            for _ in range(count_per_delay):
                ...
                data = dict()
                data[self.settings.raw_data_name] = [(noise_min - noise_max) * np.random.random_sample() + noise_max]
                ...
                self.producer.send(data, raw_data_tracker="noise_data")
