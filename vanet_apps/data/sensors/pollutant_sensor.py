...
class PollutantSensor(BaseDataSensor):
    def __init__(self, 
        ...
        print(f"[PollutantSensor] Initializing with params: {sdp.msg_per_delay}, {sdp.default_delay_s}")
    ...
    def run(self):    
        ...
        while not stop:
            ...
            for _ in range(count_per_delay):
                ...
                data = dict()
                data[self.settings.raw_data_name] = [(co_min - co_max) * np.random.random_sample() + co_max]
                ...
                self.producer.send(data, raw_data_tracker="pollutant_data")
